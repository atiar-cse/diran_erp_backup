...
class CreateProductionFormingsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('production_formings', function (Blueprint $table) {
            $table->increments('id');
            $table->string('forming_no')->unique();
            $table->date('forming_date');
            $table->string('forming_type')->nullable();
            $table->text('narration')->nullable();

            $table->decimal('total_receive_qty',18,2);
            $table->decimal('total_trans_to_shapping_qty',18,2);
            $table->decimal('total_trans_to_shap_weight',18,2);
    ...
    {
        Schema::dropIfExists('production_formings');
    }
}
