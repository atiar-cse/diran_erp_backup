...
class PurchaseOrderVoucherController extends Controller
{
    public function index(Request $request)
    { 
        if ($request->ajax()) {

            $query = DB::table('accounts_purchase_order_vouchers')
                ->leftJoin('purchase_order_receives', 'accounts_purchase_order_vouchers.purchase_stock_id', '=', 'purchase_order_receives.id')
                ->leftJoin('purchase_supplier_entries', 'accounts_purchase_order_vouchers.supplier_id', '=', 'purchase_supplier_entries.id')
                ->leftJoin('users as ura', 'accounts_purchase_order_vouchers.created_by','=','ura.id')
                ->leftJoin('users as ured', 'accounts_purchase_order_vouchers.updated_by','=','ured.id')
                ->leftJoin('users as urea', 'accounts_purchase_order_vouchers.approve_by','=','urea.id')
                ->whereNull('accounts_purchase_order_vouchers.deleted_at')
                ->select(['accounts_purchase_order_vouchers.id AS id',
                    'accounts_purchase_order_vouchers.vouchers_no',
                    'accounts_purchase_order_vouchers.purchase_date',
                    'accounts_purchase_order_vouchers.total_qty',
                    'accounts_purchase_order_vouchers.total_price',
                    'accounts_purchase_order_vouchers.price_paid',
                    'accounts_purchase_order_vouchers.price_due',
                    'accounts_purchase_order_vouchers.approve_status',
                    'accounts_purchase_order_vouchers.created_by',
                    'accounts_purchase_order_vouchers.updated_by',
                    'accounts_purchase_order_vouchers.approve_by',
        ...
        $productLists   = ProductionProducts::Select('production_products.*')
            ...
            ->get();
        $orderLists   = PurchaseOrderReceive::where('status', '1')->get();
        ...
        return view('accounts.accounts_section.purchase_invoice_voucher',
            compact( 'productLists', 'orderLists',
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($request->filled('isSuperAdmin')) {
            if ($request->isSuperAdmin == 'unapprove') {
                ...
                $this->unAppprovePurchaseInvoiceVoucher($id);

                return response()->json(['status' => 'success', 'message' => 'Purchase Invoice Voucher is successfully UnApproved!']);
                //}
        ...
        try {
            ...
            $showData = AccountsPurchaseOrderVoucher::with('get_coa','get_check_leaf')->FindOrFail($id);
            ...
            $jrData   = AccountsJournalEntry::where('transaction_reference_id',$id)
                                          ->where('transaction_reference_no','like','JPV-%')
                                          ->first();
            ...
            $data = [
                ...
                'purchase_date'     => date('d/m/Y', strtotime($showData->purchase_date)),
            ...
            $tt = $e->getMessage();
            return response()->json(['status'=>'error','data'=>$tt]);
        }
    }
    ...
    {
        try {
            $editModeData = AccountsPurchaseOrderVoucher::FindOrFail($id);
            $editModeDetailsData = AccountsPurchaseOrderVoucherDetails::where('ac_pruchase_stock_voucher_id',$id)->get();
            ...
            $query = DB::table('purchase_related_cost_entry_details as dt')
                ->leftJoin('purchase_related_cost_entries as ct', 'dt.po_cost_id', '=', 'ct.id')
                ->leftJoin('purchase_cost_types as tp', 'dt.po_cost_type_id', '=', 'tp.id')

                ->select([
                    'dt.po_cost_type_id',
                    'dt.po_cost_amount',
                    'tp.purchase_cost_types_name',
                ])
                ->where('ct.po_order_receive_id',$editModeData->purchase_stock_id)->get();
            $rel_cost = [];
            if($query){
                foreach ($query as $cot){
                    $rel_cost[]= [
                        'type_name_ct' => $cot->purchase_cost_types_name,
                        'cost_amount'  => $cot->po_cost_amount,
                    ];
                }
            }else{
                $rel_cost[]= [
            ...
            $related_cost = PurchaseRelatedCostEntry::where('po_order_receive_id',$editModeData->purchase_stock_id)->first();
            $related_cost_price = $related_cost? $related_cost->po_total_cost : 0;


            $data = [
                'id'                     => $editModeData->id,
                'vouchers_no'            => $editModeData->vouchers_no,
                'purchase_stock_id'      => $editModeData->purchase_stock_id,
                'purchase_date'          => date('d/m/Y', strtotime($editModeData->purchase_date)),
                ...
                'related_cost'           => $rel_cost,
                'related_cost_amount'    => $related_cost_price,

                'total_costing'          => ($editModeData->total_price + $related_cost_price),
            ...
            echo $e->getMessage();
            return response()->json(['status'=>'error','Something Error Happen']);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $currentId = $jid+1;
        $journalRef='JPV-'.date('Y').date('m').$currentId;

        if($due<0){ $amount =$paid; }else{ $amount =$total; }
        $data_jr = [
            'transaction_reference_id' => $id,
            'transaction_reference_no' => $journalRef,
            'transaction_date' => dateConvertFormtoDB($request->purchase_date),
            'transaction_type' => AccountsTransactionType::$purchaseInvVoucer,
            'transaction_type_name' => AccountsTransactionType::$purchaseInvVoucerTitle,
        ...
        try {
            ...
            if($approve == 1 ){
                ...
                DB::table('purchase_order_receives')->where('id', $editModeData->purchase_stock_id)->update(['account_status' => 1]);
                ...
                DB::table('accounts_purchase_order_vouchers')->where('id', $id)->update([
                    'approve_status' => 1,
            ...
            DB::commit();
            return response()->json(['status' => 'success', 'message' => 'Purchase Invoice successfully updated!']);
        }
        catch (\Exception $e) {
            DB::rollback();
    ...
    public function generateData($total, $due, $paid, $request, $id){
        if($due == 0){//Paid all amount
            $dataFormat_jr[0] = [
                'journal_entry_id' => $id ,
                'char_of_account_id' => $request->dr_chart_of_account_id, //purchase
                'remarks' => '',
    ...
    public function getPurchInvAcctNumbList(Request $request){
        return  AccountsBankAccount::where('accounts_bank_id',$request->bank_id)->get();

    }
    public function getPurchInvCheckLeafList(Request $request){
    ...
    private function unAppprovePurchaseInvoiceVoucher($id) {

        try{
            $voucher = AccountsPurchaseOrderVoucher::where('approve_status',1)
                                                    ->FindOrFail($id);
            ...
            $voucher->approve_status = 0;
            $voucher->save();

            DB::commit();
            return response()->json(['status' => 'success', 'message' => 'Purchase Order Voucher was successfully UnApproved!']);
        }
        catch(\Exception $e){
            $msg = $e->getMessage();
