...
class BankStatementExport implements FromCollection, WithHeadings, ShouldAutoSize
{
    /**
    	...
    	foreach ($ledgers as $key => $row) {
            foreach($row->details as $key2 => $row2) {
                ...
                $dataFormat[] = [
                                    ...
                                    'OPENING BALANCE'   => $row2->coa->opening_balance,
                                    'DEBIT'             => " $drData",
                                    'CREDIT'            => " $crData" ,
                                    'BALANCE'           =>number_format( $row2->coa->opening_balance+$row2->credit_amount-$row2->debit_amount,2) ,
                                ];
