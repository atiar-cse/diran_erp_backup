        ...
        <form method="POST" enctype="multipart/form-data" v-on:submit.prevent="update(form.id)" id="editComponent"
            ...
            <div class="m-portlet__body">
                ...
                <div class="form-group m-form__group row">
                    <label for="inventory_stock_adjusts_no" class="col-lg-2 col-form-label">Stock Adjust No <span
                        class="requiredField">*</span></label>
                    <div class="col-lg-4">
                        <input type="text" id="inventory_stock_adjusts_no" v-model="form.inventory_stock_adjusts_no"
                               class="form-control form-control-sm m-input" placeholder="Enter Invoice NO ">
                        <div class="requiredField" v-if="(errors.hasOwnProperty('inventory_stock_adjusts_no'))">{{
                            (errors.hasOwnProperty('inventory_stock_adjusts_no')) ? errors.inventory_stock_adjusts_no[0]
                            :'' }}
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="input-group date">
                            <input type="text" class="form-control form-control-sm m-input datepicker"
                                   v-model="form.inventory_stock_adjusts_date"
                                   data-dateField="inventory_stock_adjusts_date" readonly
                        ...
                        <div class="requiredField" v-if="(errors.hasOwnProperty('inventory_stock_adjusts_date'))">{{
                            (errors.hasOwnProperty('inventory_stock_adjusts_date')) ?
                            errors.inventory_stock_adjusts_date[0] :'' }}
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group m-form__group row">
                    ...
                    <div class="col-lg-4">
                        <select class="form-control m-select2 select2"
                                data-selectField="inventory_stock_adjusts_warehouse_id"
                                id="inventory_stock_adjusts_warehouse_id"
                                v-model="form.inventory_stock_adjusts_warehouse_id">
                            <option v-for="(wvalue,windex) in warehouse_lists" :value="wvalue.id">
                                {{wvalue.purchase_ware_houses_name}}
                            </option>
                        </select>
                        <div class="requiredField"
                             v-if="(errors.hasOwnProperty('inventory_stock_adjusts_warehouse_id'))">{{
                            (errors.hasOwnProperty('inventory_stock_adjusts_warehouse_id')) ?
                            errors.inventory_stock_adjusts_warehouse_id[0] :'' }}
                        </div>
                    </div>
                    <label class="col-lg-2 col-form-label" for="narration">Narration </label>
                    <div class="col-lg-4">
                        <textarea class="form-control m-input" v-model="form.inventory_stock_adjusts_narration"
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm m-table table-bordered borderless">
                                ...
                                <tr v-for="(details, index) in form.details">
                                    ...
                                    <td class="width-210">
                                        <select class="form-control m-select2 select2 select-product width-210"
                                                ...
                                                v-model="details.inventory_stock_adjust_details_product_id">
                                            <option v-for="(value,index) in product_lists" :value="value.id"
                                                    :measure-unit="value.measure_unit"> {{value.product_name}}
                                            </option>
                                        </select>
                                        <div class="requiredField"
                                             v-if="errors['details.'+index+'.inventory_stock_adjust_details_product_id']">
                                            {{ errors['details.'+index+'.inventory_stock_adjust_details_product_id'][0]
                                            }}
                                        </div>
                                    </td>
                                    <td>
                                        <input type="text" v-model="details.inventory_stock_adjust_details_remarks"
                                               class="form-control form-control-sm m-input" placeholder="">
                                    </td>
                                    <td class="width-210">
                                        <select class="form-control m-select2 select2 select-adjust-rule width-210"
                                                v-bind:data-index="index" v-model="details.inventory_stock_adjust_rule">
                                            <option v-for="(value,index) in enum_val" :value="value"> {{value}}</option>
                                        </select>
                                        <div class="requiredField"
                                             v-if="errors['details.'+index+'.inventory_stock_adjust_rule']">{{
                                            errors['details.'+index+'.inventory_stock_adjust_rule'][0] }}
                                        </div>
                                    </td>
                                    <td>
                                        <input type="text" v-model="details.inventory_stock_adjust_details_unit"
                                               class="form-control form-control-sm m-input" placeholder="">
                                    </td>
                                    <td>
                                        <input type="text" v-model="details.inventory_stock_adjust_details_qty"
                                               @input="totalQtyAndPrice()" class="form-control form-control-sm m-input"
                                               placeholder="">
                                        <div class="requiredField"
                                             v-if="errors['details.'+index+'.inventory_stock_adjust_details_qty']">{{
                                            errors['details.'+index+'.inventory_stock_adjust_details_qty'][0] }}
                                        </div>
                                    </td>
                                    <td>
                                        <input type="text" v-model="details.inventory_stock_adjust_details_unit_price"
                                               @input="totalQtyAndPrice()" class="form-control form-control-sm m-input">
                                        <div class="requiredField"
                                             v-if="errors['details.'+index+'.inventory_stock_adjust_details_unit_price']">
                                            {{ errors['details.'+index+'.inventory_stock_adjust_details_unit_price'][0]
                                            }}
                                        </div>
                                    </td>
                                    <td class="width-160">
                                        <input type="text" v-model="details.inventory_stock_adjust_details_total_price"
                                               class="form-control form-control-sm m-input width-160">
                                        <div class="requiredField"
                                             v-if="errors['details.'+index+'.inventory_stock_adjust_details_total_price']">
                                            {{ errors['details.'+index+'.inventory_stock_adjust_details_total_price'][0]
                                            }}
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td class="">
                                        <input type="text" v-model="form.inventory_stock_adjusts_total_qty"
                                               class="form-control form-control-sm m-input">
                                    </td>
                                    <td class="text-right">Total Amount</td>
                                    <td class="">
                                        <input type="text" v-model="form.inventory_stock_adjusts_total_price"
                                               class="form-control form-control-sm m-input">
                                    </td>
                                    <td></td>
                                </tr>
                ...
                <div class="m-form__actions m-form__actions--solid">
                    <div class="row">
                        <div class="col-lg-12 m--align-right">
                            ...
                            &lt;!&ndash;<button type="submit" class="btn btn-sm btn-success" ><i class="la la-check"></i> <span>Save</span></button>&ndash;&gt;
                            ...
                            <button v-if="permissions.indexOf('pr-requisition.approve') !=-1" type="submit"
...
<script>
    ...
    export default {
        props: ['editId', 'product_lists', 'enum_val', 'warehouse_lists'],
        data: function () {
            return {
                product_list: false,
                add_form: false,
        ...
        methods: {
            ...
            edit(id) {
                var _this = this;
                axios.get(base_url + 'stock-adjust/' + id + '/edit')
                    .then((response) => {
            ...
            update(id, app) {
                ...
                _this.$loading(true);
                axios.put(base_url + 'stock-adjust/' + id + '/update', _this.form)
                    .then((response) => {
                        _this.$loading(false);
        ...
        mounted() {
            ...
            $('#editComponent').on('change', '.select-adjust-rule', function (e) {
                var selectedType = $(this),
                    curerntVal = !!selectedType.val(),
                    dataIndex = $(e.currentTarget).attr('data-index');
