        ...
        <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="update(form.id)" id="editComponent" class="m-form m-form--fit m-form--label-align-right" >
            <div class="m-portlet__body">
                ...
                <div class="form-group m-form__group row">
                    <label for="transfers_no" class="col-lg-2 col-form-label">Stock Adjust No <span class="requiredField">*</span></label>
                    <div class="col-lg-4">
                        <input type="text"  id="transfers_no"  v-model="form.transfers_no" class="form-control form-control-sm m-input" placeholder="Enter Invoice NO " readonly>
                        <div class="requiredField" v-if="(errors.hasOwnProperty('transfers_no'))">{{ (errors.hasOwnProperty('transfers_no')) ? errors.transfers_no[0] :'' }}</div>
                    </div>
                    <label for="m_datepicker_2" class="col-lg-2 col-form-label">Return Date </label>
                    <div class="col-lg-4">
                        <div class="input-group date">
                            <input type="text" class="form-control form-control-sm m-input datepicker" v-model="form.transfers_date" data-dateField="transfers_date" readonly placeholder="Select date from picker" id="m_datepicker_2" />
                        ...
                        <div class="requiredField" v-if="(errors.hasOwnProperty('transfers_date'))">{{ (errors.hasOwnProperty('transfers_date')) ? errors.transfers_date[0] :'' }}</div>
                    </div>
                </div>
                ...
                <div class="form-group m-form__group row">
                    ...
                    <div class="col-lg-4">
                        <select class="form-control m-select2 select2" data-selectField="from_warehouse_id" id="from_warehouse_id" v-model="form.from_warehouse_id" >
                            <option v-for="(wvalue,windex) in warehouse_lists" :value="wvalue.id" > {{wvalue.purchase_ware_houses_name}}</option>
                        </select>
                        <div class="requiredField" v-if="(errors.hasOwnProperty('from_warehouse_id'))">{{ (errors.hasOwnProperty('from_warehouse_id')) ? errors.from_warehouse_id[0] :'' }}</div>
                    </div>
                    <label for="to_warehouse_id" class="col-lg-2 col-form-label">To Warehouse</label>
                    <div class="col-lg-4">
                        <select class="form-control m-select2 select2" data-selectField="to_warehouse_id" id="to_warehouse_id" v-model="form.to_warehouse_id" >
                            <option v-for="(wvalue,windex) in warehouse_lists" :value="wvalue.id" > {{wvalue.purchase_ware_houses_name}}</option>
                        </select>
                        <div class="requiredField" v-if="(errors.hasOwnProperty('to_warehouse_id'))">{{ (errors.hasOwnProperty('to_warehouse_id')) ? errors.to_warehouse_id[0] :'' }}</div>
                    </div>
                </div>
                ...
                <div class="form-group m-form__group row">
                    ...
                    <div class="col-lg-4">
                        <textarea class="form-control m-input" v-model="form.transfers_narration"  id="narration" rows="2"></textarea>
                    </div>
                    <label> </label>
                        ...
                        <div class ="table-responsive">
                            <table class="table table-sm m-table table-bordered borderless">
                                ...
                                <tr v-for="(details, index) in form.details">
                                    ...
                                    <td>
                                        <select class="form-control m-select2 select2 select-product"  v-bind:data-index="index" v-model="details.product_id"  >
                                            <option v-for="(value,index) in product_lists" :value="value.id" :measure-unit="value.measure_unit" :unit-price="value.selling_price" :current-stock="value.c_qty" > {{value.product_name}}</option>
                                        </select>
                                    ...
                                    <td>
                                        <input type="text" v-model="details.unit" class="form-control form-control-sm m-input" placeholder="">
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <input type="text" v-model="details.qty" @input="totalQtyAndPrice()" class="form-control form-control-sm m-input" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.qty']">{{ errors['details.'+index+'.qty'][0] }}</div>
                                    </td>
                                    <td>
                                        <input type="text" v-model="details.unit_price" @input="totalQtyAndPrice()" class="form-control form-control-sm m-input">
                                        <div class="requiredField" v-if="errors['details.'+index+'.unit_price']">{{ errors['details.'+index+'.unit_price'][0] }}</div>
                                    </td>
                                    <td>
                                        <input type="text" v-model="details.total_price" class="form-control form-control-sm m-input">
                                ...
                                <tr>
                                    ...
                                    <td class="">
                                        <input type="text" v-model="form.transfers_total_qty" class="form-control form-control-sm m-input">
                                    </td>
                                    <td class="text-right">Total Amount</td>
                                    <td  class="">
                                        <input type="text" v-model="form.transfers_total_price" class="form-control form-control-sm m-input">
                                    </td>
                                    <td></td>
                                </tr>
                                </tbody>
                ...
                <div class="m-form__actions m-form__actions--solid">
                    <div class="row">
                        <div class="col-lg-12 m--align-right">
                            ...
                            <button v-if="permissions.indexOf('stock-transfer.approve') !=-1" type="submit" class="btn btn-sm btn-success" @click.prevent="update(form.id,1)" ><i class="la la-check"></i> <span>Approve</span></button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        props:['permissions','editId','warehouse_lists'],
        data:function(){
            return{
                transfer_list:false,
                add_form:false,
        ...
        methods:{
            ...
            edit(id) {
                var _this = this;
                axios.get(base_url+'stock-transfer/'+id+'/edit')
                    .then( (response) => {
                        _this.form  = response.data.data.edit_mode;
                        _this.product_lists  = response.data.data.pid_list;
            ...
            update(id, app){
                ...
                axios.put(base_url+'stock-transfer/'+id+'/update',
