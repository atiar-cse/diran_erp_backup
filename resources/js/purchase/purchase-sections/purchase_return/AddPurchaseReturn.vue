                ...
                <div class="m-form__actions m-form__actions--solid">
                    <div class="row">
                        <div class="col-lg-12 m--align-right">
                            ...
                            <button v-if="permissions.indexOf('po-return.approve') !=-1" type="submit" class="btn btn-sm btn-success"  @click.prevent="store(1)"><i class="la la-check"></i> <span>Approve</span></button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    export default {
        ...
        methods:{
            transactionNoGenerate(){
                var _this = this;
                axios.get(base_url+'return-no')
                    .then( (response) => {
                        _this.form.po_return_no = response.data;
                    });
            },
            ...
            store(app){
                ...
                axios.post(base_url+'po-return/store', this.form).then( (response) => {
            ...
            totalAmount(){
                ...
                for(let i = 0; i < length; i++) {
                    ...
                    this.form.details[i].po_rtnd_total_price = total_price.toFixed(2);
                    total_qty = Number(details[i].po_rtnd_qty) + total_qty;
                    total_amount = total_price + total_amount;
                }
                this.form.po_rtn_total_qty = total_qty;
                this.form.po_rtn_total_price = total_amount.toFixed(2);
            },
            duplicateCheck(){
