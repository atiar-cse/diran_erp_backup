        ...
        <form method="POST" enctype="multipart/form-data" v-on:submit.prevent="store" id="addComponent"
            ...
            <div class="m-portlet__body">
                <div class="form-group m-form__group row">
                    <label for="purchase_requisition_no" class="col-lg-2 col-form-label">Requisition No<span
                        class="requiredField">*</span></label>
                    <div class="col-lg-4">
                        <input type="text" id="purchase_requisition_no" v-model="form.purchase_requisition_no"
                               class="form-control form-control-sm m-input" placeholder="Enter requisition no">
                        <div class="requiredField" v-if="(errors.hasOwnProperty('purchase_requisition_no'))">{{
                            (errors.hasOwnProperty('purchase_requisition_no')) ? errors.purchase_requisition_no[0] :''
                            }}
                        </div>
                    </div>
                    <label for="m_datepicker_2" class="col-lg-2 col-form-label">Requisition Date<span
                        class="requiredField">*</span></label>
                    <div class="col-lg-4">
                        <div class="input-group date">
                            <input type="text" class="form-control form-control-sm m-input datepicker"
                                   v-model="form.purchase_requisition_date" data-dateField="forming_date" readonly
                        ...
                        <div class="requiredField" v-if="(errors.hasOwnProperty('purchase_requisition_date'))">{{
                            (errors.hasOwnProperty('purchase_requisition_date')) ? errors.purchase_requisition_date[0]
                            :'' }}
                        </div>
                    </div>
                </div>
                <div class="form-group m-form__group row">
                    ...
                    <div class="col-lg-4">
                        <textarea class="form-control m-input" v-model="form.purchase_requisition_narration"
                                  id="narration" rows="2"></textarea>
                    </div>
                    <label class="col-lg-2 col-form-label"> Supervisior Narration</label>
                    <div class="col-lg-4">
                        <textarea class="form-control m-input" v-model="form.purchase_requisition_supervisor_narration"
                                  id="supervisor_narration" rows="2"
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm m-table table-bordered borderless">
                                ...
                                <tr v-for="(details, index) in form.details">
                                    ...
                                    <td class="width-210">
                                        <select class="form-control m-select2 select2  select-product width-210"
                                                v-bind:data-index="index" v-model="details.pr_product_id"
                                                v-on:change="duplicateCheck">
                                            <option v-for="(value,index) in product_lists" :value="value.id"
                                                    :measure-unit="value.measure_unit" :unit-price="value.buying_price">
                                                {{value.product_name}}
                                            </option>

                                        </select>
                                        <div class="requiredField" v-if="errors['details.'+index+'.pr_product_id']">{{
                                            errors['details.'+index+'.pr_product_id'][0] }}
                                        </div>
                                    </td>
                                    <td>
                                        <input type="text" v-model="details.pr_remarks"
                                               class="form-control form-control-sm m-input" placeholder="">
                                    </td>
                                    <td class="width-210">
                                        <select class="form-control m-select2 select2 select-product_type width-210"
                                                v-bind:data-index="index" v-model="details.product_type">
                                            <option v-for="(value,index) in enum_val" :value="value"> {{value}}</option>

                                        </select>
                                        <div class="requiredField" v-if="errors['details.'+index+'.product_type']">{{
                                            errors['details.'+index+'.product_type'][0] }}
                                        </div>
                                    </td>
                                    <td>
                                        <input type="text" v-model="details.pr_unit"
                                               class="form-control form-control-sm m-input" placeholder="" readonly>
                                    </td>
                                    <td>
                                        <input type="text" v-model="details.pr_qty" @keyup="totalQtyAndPrice()"
                                               class="form-control form-control-sm m-input" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.pr_qty']">{{
                                            errors['details.'+index+'.pr_qty'][0] }}
                                        </div>
                                    </td>
                                    <td>
                                        <input type="text" v-model="details.pr_unit_price" @keyup="totalQtyAndPrice()"
                                               class="form-control form-control-sm m-input">
                                        <div class="requiredField" v-if="errors['details.'+index+'.pr_unit_price']">{{
                                            errors['details.'+index+'.pr_unit_price'][0] }}
                                        </div>
                                    </td>
                                    <td class="width-160">
                                        <input type="text" v-model="details.pr_total_price"
                                               class="form-control form-control-sm m-input width-160" readonly>
                                        <div class="requiredField" v-if="errors['details.'+index+'.pr_total_price']">{{
                                            errors['details.'+index+'.pr_total_price'][0] }}
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td class="">
                                        <input type="text" v-model="form.purchase_requisition_total_qty"
                                               class="form-control form-control-sm m-input" readonly>
                                    </td>
                                    <td class="text-right">Total Amount</td>
                                    <td class="">
                                        <input type="text" v-model="form.purchase_requisition_total_price"
                                               class="form-control form-control-sm m-input" readonly>
                                    </td>
                                    <td></td>
                                </tr>
    ...
    export default {
        ...
        methods: {
            ...
            transactionNoGenerate() {
                var _this = this;
                _this.$loading(true);
                axios.get(base_url + 'requisition-no')
                    .then((response) => {
                        _this.$loading(false);
                        _this.form.purchase_requisition_no = response.data;
                    });
            },
            ...
            store(app) {
                ...
                _this.$loading(true);
                axios.post(base_url + 'pr-requisition/store', _this.form)
                    .then((response) => {
                        _this.$loading(false);
        ...
        mounted() {
            ...
            $('#addComponent').on('change', '.select-product_type', function (e) {
