        ...
        <div class="m-portlet__body">
            ...
            <!--<table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tr v-for="(value,index) in resultData.data">
                    <td>{{++index}}</td>
                    <td>{{value.purchase_cost_types_name}}</td>
                    <td>{{value.cost_types_code}}</td>
                    <td>
                    ...
                    <td scope="row" class="width-100 text-center">
                        <a v-if="permissions.indexOf('cost-type.edit') !=-1" @click="openUpdateModal(value.id)"  class="btn btn-outline-success m-btn m-btn&#45;&#45;icon m-btn&#45;&#45;icon-only m-btn&#45;&#45;pill m-btn&#45;&#45;air" data-offset="-20px -20px" data-toggle="m-tooltip" data-placement="top" title="Edit"><i class="la 	la-edit"></i></a>
                        <a v-if="permissions.indexOf('cost-type.destroy') !=-1" @click="deleteData(value.id)" class="btn btn-outline-danger m-btn m-btn&#45;&#45;icon m-btn&#45;&#45;icon-only m-btn&#45;&#45;pill m-btn&#45;&#45;air" data-offset="-20px -20px" data-toggle="m-tooltip" data-placement="top" title="Delete"><i class="la la-trash-o"></i></a>
                    </td>
                </tr>
                </tbody>
                <tbody v-else>
...
<script>
    ...
    import AddPurchaseCostType from './AddPurchaseCostType.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                fields: {
                    ...
                    purchase_cost_types_name: { label: 'Cost Type Name', sortable: true, searchable: true, defaultOrder: 'asc'  },
                    cost_types_code: { label: 'Cost Type Code', sortable: true, searchable: true  },
                    'accounts_chartof_accounts.chart_of_accounts_title': { label: 'COA',template:'{{ row.chart_of_accounts_title }} ({{ row.chart_of_account_code }})', sortable: true, searchable: true  },
                    'purchase_cost_types.created_by': { label: 'User', template: '<span v-if="row.created_by">Add By : {{ row.user_name }}</span><br><span v-if="row.updated_by">Edit By  : {{ row.user_name }}</span>', sortable: false, searchable: false },
                    status: {
                    ...
                    actions: {
                        isLocal: true,
                        label: 'Actions',sortable: false, searchable: false,
                        render: (data) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('cost-type.edit') != -1 ) {
                            ...
                            if (this.permissions.indexOf('cost-type.destroy') != -1) {
        ...
        methods: {
            /*index(pageNo,perPage){
                ...
                axios.get(base_url+"cost-type/?page="+pageNo+"&perPage="+perPage).then((response) => {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'cost-type/'+id+'/delete').then((response) => {
