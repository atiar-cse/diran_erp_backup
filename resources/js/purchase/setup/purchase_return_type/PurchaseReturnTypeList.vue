<template>
    <div>
        ...
        <div class="m-portlet__body">
            ...
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tr v-for="(value,index) in resultData.data">
                    <td>{{++index}}</td>
                    <td>{{value.purchase_return_types_name}}</td>
                    <td>
                    ...
                    <td scope="row" class="width-100 text-center">
                        <a v-if="permissions.indexOf('return-type.edit') !=-1"   @click="openUpdateModal(value.id)"  class="btn btn-outline-success m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air" data-offset="-20px -20px" data-toggle="m-tooltip" data-placement="top" title="Edit"><i class="la 	la-edit"></i></a>
                        <a v-if="permissions.indexOf('return-type.destroy') !=-1"   @click="deleteData(value.id)" class="btn btn-outline-danger m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air" data-offset="-20px -20px" data-toggle="m-tooltip" data-placement="top" title="Delete"><i class="la la-trash-o"></i></a>
                    </td>
                </tr>
                </tbody>
                <tbody v-else>
        ...
        <AddPurchaseReturnType></AddPurchaseReturnType>
    </div>
</template>
...
<script>
    ...
    import AddPurchaseReturnType from './AddPurchaseReturnType.vue';
    import Pagination from  '../../../components/Pagination.vue';

    export default {
        components: {
            AddPurchaseReturnType,
            Pagination
        },
        ...
        methods: {
            index(pageNo,perPage){
                ...
                axios.get(base_url+"return-type/?page="+pageNo+"&perPage="+perPage).then((response) => {
            ...
            deleteData(id){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'return-type/'+id+'/delete').then((response) => {
                            _this.index(1);
