    ...
    <div>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="">Add Return Type</h5>
                    ...
                    <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="store" class="m-form m-form--fit m-form--label-align-right">
                        ...
                        <div class="modal-body">
                            <div class="form-group m-form__group row m--margin-top-20" :class="{'has-danger': (errors.hasOwnProperty('purchase_return_types_name'))}">
                                <label class="col-form-label col-lg-4 col-sm-12">Purchase Return Type Name <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input type="text" class="form-control" v-model="form.purchase_return_types_name" placeholder="Enter purchase return type name" />
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('purchase_return_types_name'))">{{ (errors.hasOwnProperty('purchase_return_types_name')) ? errors.purchase_return_types_name[0] :'' }}</div>
                                </div>
                            </div>
        ...
        <div class="modal fade" id="editModel" role="dialog" aria-labelledby="" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit Return Type</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true" class="la la-remove"></span>
                        </button>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            store:function(){
                axios.post(base_url+'return-type/store', this.form).then( (response) => {
                    $('#addModel').modal('hide');
            ...
            update:function (id) {
                axios.put(base_url+'return-type/'+id+'/update', this.form).then( (response) => {
        ...
        mounted(){
            ...
            $('#addModel,#editModel').on('hidden.bs.modal', function(){
                _this.errors = {};
                _this.form = { 'purchase_return_types_name': '','status':'1' };
            });

            EventBus.$on('update-button-clicked', (id) => {
                _this.errors = {};
                axios.get(base_url+'return-type/'+id+'/edit').then((response) => {
