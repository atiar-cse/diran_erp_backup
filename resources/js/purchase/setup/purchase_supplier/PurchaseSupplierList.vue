...
<script>
    ...
    import AddPurchaseSupplier from './AddPurchaseSupplier.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                //VdtnetTable Options
                tableId: 'supplier-entry',
                options: {
                    ajax: {
                        url: base_url+"supplier-entry",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID'},
                    purchase_supplier_name: { label: 'Name', sortable: true, searchable: true, defaultOrder: 'DESC'  },
                    ...
                    purchase_supplier_contact_person_name: { label: 'Contact Person', sortable: true, searchable: true  },
                    purchase_supplier_business_phone: { label: 'Business Phone', sortable: true, searchable: true  },
                    purchase_supplier_mobile: { label: 'Contact Info'  },
                    purchase_supplier_office_address: { label: 'Address', sortable: false, searchable: false},
                    ...
                    'purchase_supplier_entries.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('supplier-entry.edit') != -1) {
                            ...
                            if (this.permissions.indexOf('supplier-entry.destroy') != -1) {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'supplier-entry/'+id+'/delete').then((response) => {
