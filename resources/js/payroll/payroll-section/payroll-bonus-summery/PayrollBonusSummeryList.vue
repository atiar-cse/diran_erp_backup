<template>
    <div>
        ...
        <div class="m-portlet__body" v-if="product_list">
            ...
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tr v-for="(value,index) in resultData.data">
                    <td>{{++index}}</td>
                    <td>{{formate_date(value.payroll_bonus_month)}}</td>
                    <td>{{value.payable_total_bonus}}</td>
                    ...
                    <td scope="row" class="width-100 text-center">
                        ...
                        <a v-if="permissions.indexOf('payroll-bonus-summery.edit') !=-1 && value.approve_status != 1 "  @click="editData(value.id)" class="btn btn-outline-success m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air" data-offset="-20px -20px" data-toggle="m-tooltip" data-placement="top" title="Edit"><i class="la 	la-edit"></i></a>
                    </td>
                </tr>
                </tbody>
                <tbody v-else>
        ...
        <AddPayrollBonusSummery  v-else-if="add_form"
                      :account_list ="account_list"
                      :permissions ="permissions"
        ></AddPayrollBonusSummery>

        <EditPayrollBonusSummery  v-else-if="edit_form"
                      :account_list ="account_list"
                      :permissions ="permissions"
                      :edit-id="edit_id"
        ></EditPayrollBonusSummery>
...
<script>
    ...
    import AddPayrollBonusSummery from './AddPayrollBonusSummery.vue';
    import EditPayrollBonusSummery from './EditPayrollBonusSummery.vue';
    ...
    export default {
        ...
        methods: {
            index(pageNo,perPage){
                ...
                axios.get(base_url+"payroll-bonus-summery/?page="+pageNo+"&perPage="+perPage).then((response) => {
