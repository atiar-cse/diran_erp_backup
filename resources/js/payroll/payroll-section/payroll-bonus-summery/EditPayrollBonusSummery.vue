        ...
        <form method="POST" enctype="multipart/form-data" v-on:submit.prevent="update(form.id)" id="editComponent" class="m-form m-form--fit m-form--label-align-right" >
            <div class="m-portlet__body">
                <div class="form-group m-form__group row">
                    <label for="payroll_bonus_month" class="col-lg-2 col-form-label">Bonus Month <span class="requiredField">*</span></label>
                    <div class="col-lg-4">
                        <input type="text" id="payroll_bonus_month"  class="form-control form-control-sm m-input datepicker" v-model="form.payroll_bonus_month" data-dateField="payroll_bonus_month" readonly style="width: 100%"  placeholder="Select month" autocomplete="off" />

                        <div class="requiredField" v-if="(errors.hasOwnProperty('payroll_bonus_month'))">{{ (errors.hasOwnProperty('payroll_bonus_month')) ? errors.payroll_bonus_month[0] :'' }}</div>
                    </div>
                    <label for="payable_total_bonus" class="col-lg-2 col-form-label">Total Bonus <span class="requiredField">*</span></label>
                    <div class="col-lg-4">
                        <input type="text" id="payable_total_bonus" v-model="form.payable_total_bonus" class="form-control form-control-sm m-input" placeholder="Total salary" readonly>
                        <div class="requiredField" v-if="(errors.hasOwnProperty('payable_total_bonus'))">{{ (errors.hasOwnProperty('payable_total_bonus')) ? errors.payable_total_bonus[0] :'' }}</div>
                    </div>
                </div>
                ...
                <div class="m-form__actions m-form__actions--solid">
                    <div class="row">
                        <div class="col-lg-12 m--align-right">
                            <button type="submit" class="btn btn-sm btn-info" @click.prevent="update(form.id,0)"><i class="la la-save"></i> <span>Update and Go List</span></button>
                            <button v-if="permissions.indexOf('payroll-bonus-summery.approve') !=-1" type="submit" class="btn btn-sm btn-success" @click.prevent="update(form.id,1)"><i class="la la-check"></i> <span>Approve</span></button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        props:['permissions','account_list','editId'],
        ...
        methods:{
            edit(id) {
                var _this = this;
                axios.get(base_url+'payroll-bonus-summery/'+id+'/edit')
                    .then( (response) => {
            ...
            update(id,approval){
                ...
                axios.put(base_url+'payroll-bonus-summery/'+id, this.form).then( (response) => {
            ...
            calculateTotalSalary(){
                var _this = this;
                axios.get(base_url+'calculate-total-bonus')
                    .then( (response) => {
                        _this.form.payable_total_bonus = response.data;
            ...
            $(document).on("focus", ".datepicker", function () {
                $(this).datepicker({
                    ...
                    if(ev.date == undefined){
                        if (selectField == 'payroll_bonus_month') {
                            _this.form.payroll_bonus_month = '';

                        }

                    }else if(selectField == 'payroll_bonus_month'){
                        _this.form.payroll_bonus_month = moment(ev.date).format('Y-MM-01');
