            ...
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tr v-for="(value,index) in resultData.data">
                    ...
                    <td>
                        ...
                        <div class="m-demo__preview m-demo__preview--badge" v-if="value.status==0"><span class="m-badge m-badge--danger m-badge--wide m-badge--rounded"> <i class="la la-ban"></i> Inactive</span>
                        </div>
                    </td>
                    <td class="width-100 text-center" scope="row">
                        <a @click="openUpdateModal(value.id)" class="btn btn-outline-success m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air" data-offset="-20px -20px" data-placement="top" data-toggle="m-tooltip" title="Edit" v-if="permissions.indexOf('section.edit') != -1"><i class="la 	la-edit"></i></a>
                        <a @click="deleteData(value.id)" class="btn btn-outline-danger m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air" data-offset="-20px -20px" data-placement="top" data-toggle="m-tooltip" title="Delete" v-if="permissions.indexOf('section.destroy') != -1"><i class="la la-trash-o"></i></a>
                    </td>
                </tr>
                </tbody>
                <tbody v-else>
    ...
    export default {
        ...
        methods: {
            index(pageNo, perPage) {
                ...
                axios.get(base_url + "section/?page=" + pageNo + "&perPage=" + perPage).then((response) => {
            ...
            deleteData(id) {
                ...
                swal({
                    ...
                    function () {
                        ...
                        axios.delete(base_url + 'section/' + id).then((response) => {
                            _this.index(1);
