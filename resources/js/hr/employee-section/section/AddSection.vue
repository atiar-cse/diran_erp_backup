            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="">Add Section</h5>
                    ...
                    <form class="m-form m-form--fit m-form--label-align-right" enctype="multipart/form-data" method="POST" v-on:submit.prevent="store">
                        <div class="modal-body">
                            ...
                            <div :class="{'has-danger': (errors.hasOwnProperty('section_name'))}" class="form-group m-form__group row m--margin-top-20">
                                <label class="col-form-label col-lg-4 col-sm-12">Section Name <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input class="form-control" placeholder="Enter section name" type="text" v-model="form.section_name"/>
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('section_name'))">{{(errors.hasOwnProperty('section_name')) ? errors.section_name[0] :'' }}
                                    </div>
                                </div>
                            </div>
                    ...
                    <form class="m-form m-form--fit m-form--label-align-right" enctype="multipart/form-data" method="POST" v-on:submit.prevent="update(form.id)">
                        <div class="modal-body">
                            ...
                            <div :class="{'has-danger': (errors.hasOwnProperty('section_name'))}" class="form-group m-form__group row m--margin-top-20">
                                ...
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input class="form-control" placeholder="Enter section name" type="text" v-model="form.section_name"/>
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('section_name'))">{{(errors.hasOwnProperty('section_name')) ? errors.section_name[0] :'' }}</div>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            store: function () {
                axios.post(base_url + 'section', this.form).then((response) => {
                    $('#addModel').modal('hide');
            ...
            update: function (id) {
                axios.put(base_url + 'section/' + id, this.form).then((response) => {
        ...
        mounted() {
            ...
            $('#addModel,#editModel').on('hidden.bs.modal', function () {
                _this.errors = {};
                _this.form = {'unit_id': '','section_name': '', 'status': '1'};
            ...
            EventBus.$on('update-button-clicked', (id) => {
                _this.errors = {};
                axios.get(base_url + 'section/' + id + '/edit').then((response) => {
