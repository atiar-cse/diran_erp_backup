<template>
    <div>
        ...
        <div class="m-portlet__body" v-if="product_list">
            ...
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tr v-for="(value,index) in resultData.data">
                    <td>{{++index}}</td>
                    <td>{{value.employee_terminated ? value.employee_terminated.first_name +' '+
                        value.employee_terminated.last_name:''}}
                    </td>
                    <td>{{value.termination_types}}</td>
                    <td>{{value.subject}}</td>
                    <td>{{value.terminated_by_employee ? value.terminated_by_employee.first_name + ' ' +
                        value.terminated_by_employee.last_name :''}}
                    </td>
                    <td>{{dateFormate(value.notice_date)}}</td>
                    <td>{{dateFormate(value.termination_date)}}</td>
                    <td>{{value.narration}}</td>
                    <td scope="row" class="width-100 text-center">
                        <a v-if="permissions.indexOf('termination.edit') != -1" @click="editData(value.id)"
                        ...
                        <a v-if="permissions.indexOf('termination.destroy') != -1" @click="deleteData(value.id)"
        ...
        <AddTermination v-else-if="add_form"
                        :employee_list="employee_list"
        ></AddTermination>
        <EditTermination v-else-if="edit_form"
                         :employee_list="employee_list"
                         :edit-id="edit_id"
        ></EditTermination>
    </div>
...
<script>
    ...
    import AddTermination from './AddTermination.vue';
    import EditTermination from './EditTermination.vue';
    ...
    export default {
        ...
        methods: {
            index(pageNo, perPage) {
                ...
                _this.$loading(true);
                axios.get(base_url + "termination/?page=" + pageNo + "&perPage=" + perPage)
                    .then((response) => {
                        _this.$loading(false);
            ...
            deleteData(id) {
                ...
                swal({
                    ...
                    function () {
                        ...
                        _this.$loading(true);
                        axios.delete(base_url + 'termination/' + id)
                            .then((response) => {
                                _this.$loading(false);
                                _this.index(1);
