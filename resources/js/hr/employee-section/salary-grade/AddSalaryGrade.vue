            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="">Add Salary Grade</h5>
                    ...
                    <form method="POST" enctype="multipart/form-data" v-on:submit.prevent="store"
                        ...
                        <div class="modal-body">
                            <div class="form-group m-form__group row m--margin-top-20"
                                 :class="{'has-danger': (errors.hasOwnProperty('title'))}">
                                <label class="col-form-label col-lg-4 col-sm-12">Grade <span
                                    class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input class="form-control" placeholder="Enter Grade" type="text"
                                           v-model="form.title"/>
    ...
    export default {
        ...
        methods: {
            store: function () {
                var _this = this;
                _this.$loading(true);
                axios.post(base_url + 'salary-grade', _this.form)
                    .then((response) => {
                        _this.$loading(false);
                        $('#addModel').modal('hide');
            ...
            update: function (id) {
                var _this = this;
                _this.$loading(true);
                axios.put(base_url + 'salary-grade/' + id, _this.form)
                    .then((response) => {
                        _this.$loading(false);
        ...
        mounted() {
            ...
            $('#addModel,#editModel').on('hidden.bs.modal', function () {
                _this.errors = {};
                _this.form = {'title': '', 'status': '1'};
            });

            EventBus.$on('update-button-clicked', (id) => {
                _this.errors = {};
                _this.$loading(true);
                axios.get(base_url + 'salary-grade/' + id + '/edit')
                    .then((response) => {
                        _this.$loading(false);
