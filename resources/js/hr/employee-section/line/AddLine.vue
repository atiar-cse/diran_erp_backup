            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="">Add Line</h5>
                    ...
                    <form class="m-form m-form--fit m-form--label-align-right" enctype="multipart/form-data" method="POST" v-on:submit.prevent="store">
                        ...
                        <div class="modal-body">
                            ...
                            <div :class="{'has-danger': (errors.hasOwnProperty('line_name'))}" class="form-group m-form__group row m--margin-top-20">
                                <label class="col-form-label col-lg-4 col-sm-12">Line Name <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input class="form-control" placeholder="Enter line name" type="text" v-model="form.line_name"/>
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('line_name'))">{{(errors.hasOwnProperty('line_name')) ? errors.line_name[0] :'' }}</div>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            store: function () {
                axios.post(base_url + 'line', this.form).then((response) => {
                    $('#addModel').modal('hide');
            ...
            update: function (id) {
                axios.put(base_url + 'line/' + id, this.form).then((response) => {
        ...
        mounted() {
            ...
            $('#addModel,#editModel').on('hidden.bs.modal', function () {
                _this.errors = {};
                _this.form = {'unit_id': '', 'line_name': '', 'status': '1'};
            ...
            EventBus.$on('update-button-clicked', (id) => {
                _this.errors = {};
                axios.get(base_url + 'line/' + id + '/edit').then((response) => {
