            ...
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tr v-for="(value,index) in resultData.data">
                    ...
                    <td class="width-100 text-center" scope="row">
                        <a @click="openUpdateModal(value.id)" class="btn btn-outline-success m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air" data-offset="-20px -20px" data-placement="top" data-toggle="m-tooltip" title="Edit" v-if="permissions.indexOf('line.edit') != -1"><i class="la 	la-edit"></i></a>
                        <a @click="deleteData(value.id)" class="btn btn-outline-danger m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air" data-offset="-20px -20px" data-placement="top" data-toggle="m-tooltip" title="Delete" v-if="permissions.indexOf('line.destroy') != -1"><i class="la la-trash-o"></i></a>
                    </td>
                </tr>
                </tbody>
                <tbody v-else>
    ...
    export default {
        ...
        methods: {
            index(pageNo, perPage) {
                ...
                axios.get(base_url + "line/?page=" + pageNo + "&perPage=" + perPage).then((response) => {
            ...
            deleteData(id) {
                ...
                swal({
                    ...
                    function () {
                        swal("Deleted!", "Your Line Name has been deleted.", "success");
                        axios.delete(base_url + 'line/' + id).then((response) => {
                            _this.index(1);
