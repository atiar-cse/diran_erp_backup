            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="">Add Floor</h5>
                    ...
                    <form class="m-form m-form--fit m-form--label-align-right" enctype="multipart/form-data" method="POST" v-on:submit.prevent="store">
                        ...
                        <div class="modal-body">
                            ...
                            <div :class="{'has-danger': (errors.hasOwnProperty('floor_name'))}" class="form-group m-form__group row m--margin-top-20">
                                <label class="col-form-label col-lg-4 col-sm-12">Floor Name <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input class="form-control" placeholder="Enter floor name" type="text" v-model="form.floor_name"/>
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('floor_name'))">{{(errors.hasOwnProperty('floor_name')) ? errors.floor_name[0] :'' }}</div>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            store: function () {
                axios.post(base_url + 'floor', this.form).then((response) => {
                    $('#addModel').modal('hide');
            ...
            update: function (id) {
                axios.put(base_url + 'floor/' + id, this.form).then((response) => {
        ...
        mounted() {
            ...
            $('#addModel,#editModel').on('hidden.bs.modal', function () {
                _this.errors = {};
                _this.form = {'unit_id': '', 'floor_name': '', 'status': '1'};
            ...
            EventBus.$on('update-button-clicked', (id) => {
                _this.errors = {};
                axios.get(base_url + 'floor/' + id + '/edit').then((response) => {
