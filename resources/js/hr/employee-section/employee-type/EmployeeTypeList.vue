            ...
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tr v-for="(value,index) in resultData.data">
                    ...
                    <td scope="row" class="width-100 text-center">
                        <a v-if="permissions.indexOf('employee-type.edit') != -1" @click="openUpdateModal(value.id)"  class="btn btn-outline-success m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air" data-offset="-20px -20px" data-toggle="m-tooltip" data-placement="top" title="Edit"><i class="la 	la-edit"></i></a>
                        <a v-if="permissions.indexOf('employee-type.destroy') != -1" @click="deleteData(value.id)" class="btn btn-outline-danger m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air" data-offset="-20px -20px" data-toggle="m-tooltip" data-placement="top" title="Delete"><i class="la la-trash-o"></i></a>
                    </td>
                </tr>
                </tbody>
                <tbody v-else>
    ...
    export default {
        ...
        methods: {
            index(pageNo,perPage){
                ...
                axios.get(base_url+"employee-type/?page="+pageNo+"&perPage="+perPage).then((response) => {
            ...
            deleteData(id){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'employee-type/'+id).then((response) => {
                            _this.index(1);
