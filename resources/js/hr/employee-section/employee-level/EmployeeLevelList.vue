<template>
    <div>
        <div class="m-portlet__body">
            ...
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tr v-for="(value,index) in resultData.data">
                    <td>{{++index}}</td>
                    <td>{{value.employee_level_name}}</td>
                    <td>{{value.order}}</td>
                    <td>
                    ...
                    <td class="width-100 text-center" scope="row">
                        <a @click="openUpdateModal(value.id)"
                           ...
                           data-offset="-20px -20px" data-placement="top" data-toggle="m-tooltip" title="Edit"
                           v-if="permissions.indexOf('employee-level.edit') != -1"><i class="la 	la-edit"></i></a>
                        <a @click="deleteData(value.id)"
                           class="btn btn-outline-danger m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air"
                           data-offset="-20px -20px" data-placement="top" data-toggle="m-tooltip" title="Delete"
                           v-if="permissions.indexOf('employee-level.destroy') != -1"><i class="la la-trash-o"></i></a>
                    </td>
                </tr>
                </tbody>
                <tbody v-else>
        ...
        <AddEmployeeLevel></AddEmployeeLevel>
    </div>
</template>
...
<script>
    ...
    import AddEmployeeLevel from './AddEmployeeLevel.vue';
    ...
    export default {
        ...
        methods: {
            index(pageNo, perPage) {
                ...
                axios.get(base_url + "employee-level/?page=" + pageNo + "&perPage=" + perPage).then((response) => {
            ...
            deleteData(id) {
                ...
                swal({
                    ...
                    function () {
                        ...
                        axios.delete(base_url + 'employee-level/' + id).then((response) => {
                            _this.index(1);
