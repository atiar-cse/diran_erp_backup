    ...
    <div>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Add Weekly Holiday</h5>
                    ...
                    <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="store" class="m-form m-form--fit m-form--label-align-right">
                        <div class="modal-body">
                            <div class="form-group m-form__group row m--margin-top-20" :class="{'has-danger': (errors.hasOwnProperty('product_category_name'))}">
                                <label class="col-form-label col-lg-4 col-sm-12" for="weekly_holiday_names">Weekly Holiday Name <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <select class="form-control select2" id="weekly_holiday_names"  v-model="form.weekly_holiday_name" data-selectField="weekly_holiday_name" style="width:100% ">
                                    ...
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('weekly_holiday_name'))">{{ (errors.hasOwnProperty('weekly_holiday_name')) ? errors.weekly_holiday_name[0] :'' }}</div>
                                </div>
                            </div>
        ...
        <div class="modal fade" id="editModel" role="dialog" aria-labelledby="" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit Weekly Holiday </h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true" class="la la-remove"></span>
                        </button>
                    </div>
                    <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="update(form.id)" class="m-form m-form--fit m-form--label-align-right">
                        <div class="modal-body">
                            <div class="form-group m-form__group row m--margin-top-20" :class="{'has-danger': (errors.hasOwnProperty('product_category_name'))}">
                                <label class="col-form-label col-lg-4 col-sm-12" for="weekly_holiday_name">Weekly Holiday Name <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <select class="form-control select2" id="weekly_holiday_name"  v-model="form.weekly_holiday_name" data-selectField="weekly_holiday_name" style="width:100% ">
                                        <option value="Friday">Friday</option>
                                        <option value="Saturday">Saturday</option>
                                        <option value="Sunday">Sunday</option>
                                        <option value="Monday">Monday</option>
...
<script>
    ...
    export default {
        ...
        methods:{
            store:function(){
                axios.post(base_url+'weekly-holiday', this.form).then( (response) => {
                    $('#addModel').modal('hide');
            ...
            update:function (id) {
                axios.put(base_url+'weekly-holiday/'+id, this.form).then( (response) => {
        ...
        mounted(){
            ...
            $('#addModel,#editModel').on('change', '.select2', function (e) {
                ...
                if( selectField == 'weekly_holiday_name' ){
                    _this.form.weekly_holiday_name= selectedItem.val();
                }
            ...
            $('#addModel,#editModel').on('hidden.bs.modal', function(){
                _this.errors = {};
                _this.form = {'weekly_holiday_name':'','status':'1'};
            });

            EventBus.$on('update-button-clicked', (id) => {
                _this.errors = {};
                axios.get(base_url+'weekly-holiday/'+id+'/edit').then((response) => {
