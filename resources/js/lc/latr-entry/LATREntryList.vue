    ...
    <div>
        <div class="m-portlet__body" v-if="product_list">
            ...
            <div class="table-responsive">
                <vdtnet-table :id="tableId" ref="table" :fields="fields" :opts="options" @edit="doAlertEdit" @delete="doAlertDelete" @view="doAlertView">
                </vdtnet-table>
            </div>
        </div>

        <AddLATREntry v-else-if="add_form" 
                :permissions="permissions" 
                :lc_lists="lc_lists" 
                :product_lists="product_lists" 
                :measure_unit_lists="measure_unit_lists"
        ></AddLATREntry>
        <EditLATREntry v-else-if="edit_form" 
                :permissions="permissions" 
                :lc_lists="lc_lists" 
                :product_lists="product_lists" 
                :measure_unit_lists="measure_unit_lists"
...
<script>
    ...
    import AddLATREntry from './AddLATREntry.vue';
    import EditLATREntry from './EditLATREntry.vue';
    import ViewLATREntry from './ViewLATREntry.vue';
    ...
    export default {
        ...
        props:['permissions','lc_lists','product_lists','measure_unit_lists'],
        ...
        data: function(){
            return {
                ...
                fields: {
                    ...
                    latr_date: { label: 'LTR Date', sortable: true, searchable: true },
                    lc_margin_percentage: { label: 'LC Margin (%)' },
                    total_qty: { label: 'Qty'  },
                    total_amount: { label: 'Total Amount (BDT)' },
                    latr_percentage: { label: 'LTR (%)' },
                    latr_total_amount: { label: 'LTR Amount'  },
                    ...
                    'lc_latr_entries.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            ...
                            if (this.permissions.indexOf('latr-entry.edit') != -1 && row.approve_status !=1 ) {
                            ...
                            if (this.permissions.indexOf('latr-entry.destroy') != -1 && row.approve_status !=1 ) {
                            ...
                            if (this.permissions.indexOf('latr-entry.show') != -1 ) {
        ...
        methods: {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        swal("Deleted!", "Your Latr Entry has been deleted.", "success");
                        axios.delete(base_url+'latr-entry/'+id).then((response) => {
