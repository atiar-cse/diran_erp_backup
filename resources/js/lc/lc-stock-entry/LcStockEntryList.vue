<template>
    <div>
        ...
        <AddLcStockEntry v-else-if="add_form" 
        ...
        ></AddLcStockEntry>
        <EditLcStockEntry v-else-if="edit_form" 
            :permissions="permissions"  
            :ci_lists="ci_lists"  
            :product_lists="product_lists"
            :measure_unit_lists="measure_unit_lists"
...
<script>
    ...
    import AddLcStockEntry from './AddLcStockEntry.vue';
    import EditLcStockEntry from './EditLcStockEntry.vue';
    import ViewLcStockEntry from './ViewLcStockEntry.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'lc-stock-entry',
                options: {
                    ajax: {
                        url: base_url+"lc-stock-entry",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    ...
                    total_qty: { label: 'Qty' },
                    total_net_weight: { label: 'Net Weight' },
                    ...
                    'lc_stock_entries.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            ...
                            if (this.permissions.indexOf('lc-stock-entry.edit') != -1 && row.approve_status !=1 ) {
                            ...
                            if (this.permissions.indexOf('lc-stock-entry.destroy') != -1 && row.approve_status !=1 ) {
                            ...
                            if (this.permissions.indexOf('lc-stock-entry.show') != -1 ) {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'lc-stock-entry/'+id).then((response) => {
