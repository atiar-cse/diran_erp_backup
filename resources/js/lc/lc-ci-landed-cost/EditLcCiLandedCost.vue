<template>
    <div>
        <!--begin::Form-->
        <form method="POST" enctype="multipart/form-data" v-on:submit.prevent="update()" id="editComponent" class="m-form m-form--fit m-form--label-align-right"  >
                ...
                <div class="m-form__actions m-form__actions--solid">
                    <div class="row">
                        <div class="col-lg-12 m--align-right">
                            <button v-if="permissions.indexOf('lc-ci-landed-cost.approve') !=-1" type="submit" class="btn btn-sm btn-success" @click.prevent="update()"><i class="la la-check"></i> <span>Approve</span></button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            update(){
                var _this = this;
                _this.form.approval = 1;   
                var ci_id = _this.form.lc_commercial.id; 

                var result = confirm("Are you sure? PER UNIT COST WITH VAT will be counted!");
                if (result) {
                    axios.put(base_url+'lc-ci-landed-cost/'+ci_id, this.form).then( (response) => {
