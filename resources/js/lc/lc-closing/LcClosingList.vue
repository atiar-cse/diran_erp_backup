<template>
    <div>
        ...
        <AddLcClosing v-else-if="add_form" 
            :permissions="permissions" 
            :lc_lists="lc_lists"
            :account_list="account_list"
        ></AddLcClosing>
<!--         <EditLcClosing v-else-if="edit_form" 
            :permissions="permissions"  
            :lc_lists="lc_lists"  
            :account_list="account_list"
            :edit-id="edit_id" 
...
<script>
    ...
    import AddLcClosing from './AddLcClosing.vue';
    // import EditLcClosing from './EditLcClosing.vue';
    import ViewLcClosing from './ViewLcClosing.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                fields: {
                    ...
                    closing_date: { label: 'Closing Date', sortable: true, searchable: true },
                    'lc_opening_sections.lc_total_value': { label: 'LC Total Value',template:'{{ row.lc_total_value }}', sortable: true, searchable: true},
                    'lc_opening_sections.lc_opening_charges': { label: 'Opening Charges',template:'{{ row.lc_opening_charges }}', sortable: true, searchable: true},
                    'lc_opening_sections.lc_bank_expenses': { label: 'Bank Expense',template:'{{ row.lc_bank_expenses }}', sortable: true, searchable: true},
                    'lc_opening_sections.insurance_amount': { label: 'Insurance',template:'{{ row.insurance_amount }}', sortable: true, searchable: true},
                    total_cost: { label: 'Total Cost (BDT)'  },
                    ...
                    'lc_closings.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            ...
                            if (this.permissions.indexOf('lc-closing.edit') != -1 && row.approve_status !=1 ) {
                            ...
                            if (this.permissions.indexOf('lc-closing.destroy') != -1 && row.approve_status !=1 ) {
                            ...
                            if (this.permissions.indexOf('lc-closing.show') != -1 ) {
        ...
        methods: {
            /*index(pageNo,perPage){
                ...
                axios.get(base_url+"lc-closing/?page="+pageNo+"&perPage="+perPage).then((response) => {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'lc-closing/'+id).then((response) => {
