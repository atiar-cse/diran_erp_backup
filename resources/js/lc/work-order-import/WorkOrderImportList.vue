<template>
    <div>
        ...
        <AddWorkOrderImport v-else-if="add_form" 
            ...
            :coa_lists="coa_lists"
        ></AddWorkOrderImport>
        <EditWorkOrderImport v-else-if="edit_form" 
            :permissions="permissions" 
            :product_lists="product_lists" 
            :edit-id="edit_id" 
            :supplier_lists="supplier_lists"
...
<script>
    ...
    import AddWorkOrderImport from './AddWorkOrderImport.vue';
    import EditWorkOrderImport from './EditWorkOrderImport.vue';
    import ViewWorkOrderImport from './ViewWorkOrderImport.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'work-order-import',
                options: {
                    ajax: {
                        url: base_url+"work-order-import",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID'},
                    work_order_no: { label: 'Work Order  No', sortable: true, searchable: true, defaultOrder: 'DESC'  },
                    ...
                    order_date: { label: 'Order Date', sortable: true, searchable: true  },
                    total_qty: { label: 'Qty' },
                    ...
                    'lc_work_order_imports.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            ...
                            if (this.permissions.indexOf('work-order-import.edit') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permissions.indexOf('work-order-import.destroy') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permissions.indexOf('work-order-import.show') != -1) {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'work-order-import/'+id).then((response) => {
