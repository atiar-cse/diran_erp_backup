<template>
    <div>
        <div class="m-portlet__body">
        <div class="row print_links">
            <div class="col-md-10"></div>
            <div class="col-md-2 text-right">
                <a href="#" onclick="window.print();return false"
                   class="btn btn-primary btn-sm m-btn  m-btn m-btn--icon"><span><i
                        class="fa flaticon-technology"></i><span>Print</span></span></a>
                <!--<a href="#" class="btn btn-accent btn-sm m-btn  m-btn m-btn&#45;&#45;icon"><span><i class="fa flaticon-doc"></i><span>PDF</span></span></a>-->
            </div>
        </div>
        <br>

        <div class="row">
            <div class="col-lg-12">
                <div class="m-portlet">
                    <div class="m-portlet__body m-portlet__body--no-padding">
                        <div class="m-invoice-2">
                            <div class="m-invoice__wrapper">
                                <div class="m-invoice__head">
                                    <div class="m-invoice__container m-invoice__container--centered">
                                        <div class="row invoice-header" style="padding-top: 24px">
                                            <div class="col-md-9">
                                                <div class="db-data">
                                                    <h1 class="print-head-title">LC Commercial Invoice</h1>
                                                    <p v-if="form.approve_status == 0" class="m--font-danger"><span
                                                            style="font-weight: 600">Awaiting Approval</span>
                                                        <span> </span></p>
                                                    <p v-if="form.approve_status == 1" class="m--font-success"><span
                                                            style="font-weight: 600">Approved</span> <span> </span></p>
                                                </div>
                                            </div>
                                            <div class="col-md-3 col-md-offset-3">
                                                <p style="margin-bottom: 62px;">
                                                    <!--<img src="img/1200px-Channel-i.svg.png" height="30px" width="30px">-->
                                                    <img :src="cinfo.logo" style="width: 150px; height: 50px;float: right">
                                                </p>
                                                <span class="m-invoice__desc">
                                                         <h3><span>{{cinfo.name}}</span></h3>
                                                        <span>{{cinfo.address}}</span>
                                                    </span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="m-invoice__body m-invoice__body--centered">
                                    <div class="description-section">
                                        <div class="row">
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">LC No :</span>
                                                    <span>
                                                        {{form.get_lc_no ? form.get_lc_no.lc_no : ''}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">CI B/L No :</span>
                                                    <span>
                                                        {{form.ci_no}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Shipping Mode :</span>
                                                    <span v-if="form.shipping_mode == 1"
                                                          class="badge badge-pill badge-info">Truck</span>
                                                    <span v-if="form.shipping_mode == 2"
                                                          class="badge badge-pill badge-warning">Sea</span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Receive Date :</span>
                                                    <span>
                                                        {{form.receive_date}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">CI B/L Date :</span>
                                                    <span>
                                                        {{form.ci_bill_entry_date}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">C&F Agent :</span>
                                                    <span>
                                                        {{form.get_cnf_agent ? form.get_cnf_agent.cnf_agent_name : ''}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Invoice No :</span>
                                                    <span>
                                                        {{form.invoice_no}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">CI Bill of Entry No :</span>
                                                    <span>
                                                        {{form.ci_bill_of_entry_no}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Exp. No :</span>
                                                    <span>
                                                        {{form.exp_no}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Invoice Date :</span>
                                                    <span>
                                                        {{form.invoice_date}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">CI Bill of Entry Date :</span>
                                                    <span>
                                                        {{form.ci_bill_entry_date}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">ETA Date :</span>
                                                    <span>
                                                        {{form.eta_date}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Invoice Amount :</span>
                                                    <span>
                                                        {{form.invoice_amount}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Vassel Name :</span>
                                                    <span>
                                                        {{form.vassel_name}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Shipping Line :</span>
                                                    <span>
                                                        {{form.shipping_line}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Depart From :</span>
                                                    <span>
                                                        {{form.depart_from}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Arrived Port :</span>
                                                    <span>
                                                        {{form.arrived_port}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Present Destination :</span>
                                                    <span>
                                                        {{form.present_destination}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Origin :</span>
                                                    <span>
                                                        {{form.get_origin ? form.get_origin.country_name : ''}}
                                                    </span>
                                                </p>
                                            </div>
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Narration :</span>
                                                    <span>
                                                        {{form.narration}}
                                                    </span>
                                                </p>
                                            </div>

                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Delivery Type :</span>
                                                    <span v-if="form.ci_delivery_type == 1"
                                                          class="badge badge-pill badge-info">All Products</span>
                                                    <span v-if="form.ci_delivery_type == 2"
                                                          class="badge badge-pill badge-warning">Partial Products</span>
                                                </p>
                                            </div>
                                            
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Goods Name :</span>
                                                    <span>{{ form.goods_name }}</span>
                                                </p>
                                            </div> 
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Margin Amount :</span>
                                                    <span>{{ form.ci_margin }}</span>
                                                </p>
                                            </div> 
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">LC Opening Charge :</span>
                                                    <span>{{ form.ci_opening_charge }}</span>
                                                </p>
                                            </div> 
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Bank Expenses :</span>
                                                    <span>{{ form.ci_bank_expenses }}</span>
                                                </p>
                                            </div> 
                                            <div class="col-md-6" style="float:left;width:50%">
                                                <p>
                                                    <span style="font-weight: 600">Insurance Amount :</span>
                                                    <span>{{ form.ci_insurance }}</span>
                                                </p>
                                            </div>                                            
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="table-responsive">
                                                    <table class="table table-bordered">
                                                        <thead>
                                                        <tr>
                                                            <th>SL</th>
                                                            <th>Product</th>
                                                            <th>H.S Code</th>
                                                            <th>LC Unit Price (BDT)</th>
                                                            <th>Qty</th>
                                                            <th>Unit</th>
                                                            <th>Gross Weight</th>
                                                            <th>Net Weight</th>
                                                            <th>Total Amount (BDT)</th>
                                                        </tr>
                                                        </thead>
                                                        <tbody>
                                                        <tr v-for="(details, index) in form.details">
                                                            <td scope="row">
                                                                {{++index}}
                                                            </td>
                                                            <td>
                                                                {{details.get_product ? details.get_product.product_name : ''}}
                                                            </td>
                                                            <td>
                                                                {{details.hs_code}}
                                                            </td>
                                                            <td>
                                                                {{details.unit_price}}
                                                            </td>
                                                            <td>
                                                                {{details.qty}}
                                                            </td>
                                                            <td>
                                                                {{details.get_measure_unit ? details.get_measure_unit.measure_unit : ''}}
                                                            </td>
                                                            <td>
                                                                {{details.gross_weight}}
                                                            </td>
                                                            <td>
                                                                {{details.net_weight}}
                                                            </td>
                                                            <td>
                                                                {{details.total_amount}}
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td colspan="4" class="text-right">Total </td>
                                                            <td class="">
                                                                {{form.total_qty}}
                                                            </td>
                                                            <td></td>
                                                            <td class="">
                                                                {{form.total_gross_weight}}
                                                            </td>
                                                            <td class="">
                                                                {{form.total_net_weight}}
                                                            </td>
                                                            <td>
                                                                {{form.total_amount}}
                                                            </td>
                                                        </tr>
                                                        </tbody>
                                                    </table>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

    </div>
    </div>

</template>

<script>
    import {EventBus} from '../../vue-assets';
    export default {

        props: ['editId'],
        data: function () {
            return {
                product_list: false,
                add_form: false,
                edit_form: false,
                view_form: true,
                cinfo: {},

                form: {
                    lc_opening_info_id: '',
                    ci_no: '',
                    ci_date: '',
                    ci_bill_of_entry_no: '',
                    ci_bill_entry_date: '',
                    invoice_no: '',
                    invoice_date: '',
                    invoice_amount: '',
                    eta_date: '',
                    cnf_agent: '',
                    shipping_mode: '',
                    exp_no: '',
                    vassel_name: '',
                    shipping_line: '',
                    receive_date: '',
                    depart_from: '',
                    arrived_port: '',
                    present_destination: '',
                    origin: '',
                    narration: '',
                    total_qty: '',
                    total_net_weight: '',
                    total_gross_weight: '',
                    total_amount: '',
                    status: 1,
                    approve: 0,
                    details: [
                        {
                            product_id: '',
                            hs_code: '',
                            unit_price: '',
                            qty: '',
                            measure_unit_id: '',
                            gross_weight: '',
                            net_weight: '',
                            total_amount: 0,
                        }
                    ],
                },
                errors: {},
            };
        },

        methods: {

            company_info(){
                var _this = this;
                var id = 1;
                axios.get(base_url + 'Company-Info/' + id)
                    .then((response) => {
                        _this.cinfo = response.data;
                    });
            },

            show(id) {
                var _this = this;
                axios.get(base_url + 'commercial-invoice-entry/' + id)
                    .then((response) => {
                        _this.form = response.data.data;
                    });
            },

        },

        mounted(){
            var _this = this;

        },

        created(){
            var _this = this;
            _this.show(_this.editId);
            _this.company_info();

        }

    }
</script>
