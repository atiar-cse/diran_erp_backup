<template>
    <div>
        ...
        <AddCommercialInvoiceEntry v-else-if="add_form" 
            :lc_lists="lc_lists" 
            :permissions="permissions" 
            :cnf_agent_list="cnf_agent_list"
            :product_lists="product_lists"
            :country_lists="country_lists"
            :measure_unit_lists="measure_unit_lists"
        ></AddCommercialInvoiceEntry>
        <EditCommercialInvoiceEntry v-else-if="edit_form" 
            :lc_lists="lc_lists"  
            :permissions="permissions"  
            :cnf_agent_list="cnf_agent_list"
            :product_lists="product_lists"
            ...
            :measure_unit_lists="measure_unit_lists"
            :edit-id="edit_id" 
        ></EditCommercialInvoiceEntry>
        <ViewCommercialInvoiceEntry
            v-else-if="view_form"
            :edit-id="edit_id"
        ></ViewCommercialInvoiceEntry>
    </div>
...
<script>
    ...
    import AddCommercialInvoiceEntry from './AddCommercialInvoiceEntry.vue';
    import EditCommercialInvoiceEntry from './EditCommercialInvoiceEntry.vue';
    import ViewCommercialInvoiceEntry from './ViewCommercialInvoiceEntry.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'commercial-invoice-entry',
                options: {
                    ajax: {
                        url: base_url+"commercial-invoice-entry",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID'  },
                    ci_no: { label: 'CI B/L No', sortable: true, searchable: true , defaultOrder:'DESC'},
                    receive_date: { label: 'Receive Date', sortable: true, searchable: true },
                    'lc_opening_sections.lc_no': { label: 'LC No',template:'{{ row.lc_no }}', sortable: true, searchable: true},
                    invoice_no: { label: 'Invoice No', sortable: true, searchable: true},
                    total_qty: { label: 'Qty'  },
                    total_amount: { label: 'Amount (BDT)'  },
                    shipping_mode: { label: 'User', template: '<span v-if="row.shipping_mode == 1" class="badge badge-pill badge-info">Truck</span> <span v-if="row.shipping_mode == 2" class="badge badge-pill badge-warning">Sea</span> <span v-if="row.shipping_mode == 3" class="badge badge-pill badge-success">Air</span>' },
                    ...
                    'lc_cf_value_margin_entries.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            ...
                            if (this.permissions.indexOf('commercial-invoice-entry.edit') != -1 && row.approve_status !=1 ) {
                            ...
                            if (this.permissions.indexOf('commercial-invoice-entry.destroy') != -1 && row.approve_status !=1 ) {
                            ...
                            if (this.permissions.indexOf('commercial-invoice-entry.show') != -1 ) {
        ...
        methods: {
            /*index(pageNo,perPage){
                ...
                axios.get(base_url+"commercial-invoice-entry/?page="+pageNo+"&perPage="+perPage).then((response) => {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'commercial-invoice-entry/'+id).then((response) => {
