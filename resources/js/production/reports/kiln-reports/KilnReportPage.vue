<template>
    <div>
        <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="kilns" id="listComponent" class="m-form m-form--fit m-form--label-align-right hide_print" >
        ...
        <div class="m-portlet__body" v-if="kiln_report_list">
            ...
            <div class="m-invoice__head print_only">
                <div class="m-invoice__container m-invoice__container--centered">
                    <div class="row invoice-header"  style="padding-top: 0px;padding-bottom:15px;">
                        <div class="col-md-4">
                            <div class="db-data">
                                <p><span style="font-weight: 600">Section :</span> <span>Kiln</span></p>
            ...
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tbody v-if="resultData !=''">
                    <tr v-for="(value,index) in resultData.prod_kiln_details">
                        <td>{{++index}}</td>
                        <td>{{value.product.product_name}}</td>
                        <td>{{value.opening_qty}}</td>
                        <td>{{value.rec_f_gla}}</td>
                        <td>{{formatNumber(value.total)}}</td>
                        <td>{{value.trans_to_hv_qty}}</td>
    ...
    export default {
        ...
        methods: {
            ...
            kilns: function () {
                ...
                axios.get(base_url+"kiln-report/?from_date="+from_date+"&to_date="+to_date).then((response) => {
            ...
            formatData(){
                ...
                var prod_kiln_details    = _this.resultData.prod_kiln_details;
                prod_kiln_details.forEach(function(item) {

                    item.opening_qty = opening_products_qty[item.product_id];
                    item.total = parseFloat(item.rec_f_gla) + parseFloat(item.opening_qty);
                    item.total_issue = parseFloat(item.trans_to_hv_qty) + parseFloat(item.reject_qty);
