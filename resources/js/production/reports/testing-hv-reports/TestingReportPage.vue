<template>
    <div>
        ...
        <div class="m-portlet__body" v-if="hv_report_list">
            ...
            <div class="m-invoice__head print_only">
                <div class="m-invoice__container m-invoice__container--centered">
                    <div class="row invoice-header"  style="padding-top: 0px;padding-bottom:15px;">
                        <div class="col-md-4">
                            <div class="db-data">
                                <p><span style="font-weight: 600">Section :</span> <span>H/V</span></p>
            ...
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tbody v-if="resultData !=''">
                    <tr v-for="(value,index) in resultData.prod_testing_details">
                        ...
                        <td>{{value.rec_f_kiln}}</td>
                        <td>{{value.adj_qty}}</td>
                        <td>{{value.total}}</td>
                        <td>{{value.reject_qty}}</td>

                        <td>{{value.trans_to_finishing}}</td>
                        <td>{{value.total_issue}}</td>
                        <td>{{Number(value.balance) + Number(value.adj_qty)}}</td>
    ...
    export default {
        ...
        methods: {
            ...
            search: function () {
                ...
                axios.get(base_url+"testing-hv-report/?from_date="+from_date+"&to_date="+to_date).then((response) => {
            ...
            formatData(){
                ...
                prod_testing_details.forEach(function(item) {
                    ...
                    item.total = parseFloat(item.rec_f_kiln) + parseFloat(item.opening_qty);
                    item.total_issue = parseFloat(item.trans_to_finishing) + parseFloat(item.reject_qty);
