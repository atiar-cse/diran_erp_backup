        ...
        <div id="m-portlet" class="m-portlet__body" v-if="sales_report_list">
            ...
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tbody v-if="resultData !=''">
                    <tr v-for="(value,index) in resultData.finished_items">
                        <td>{{++index}}</td>
        ...
        methods: {
            ...
            formatData(){
                ...
                /*prod_shapping_details.forEach(function(item) {
                    ...
                    item.trans_to_store_qty = parseFloat(finished_items[product_id]) + parseFloat(assembled_items[product_id]);
                    item.reject_qty = parseFloat(rejected_items[product_id]);

                    item.total_issue = parseFloat(item.trans_to_store_qty) + parseFloat(item.reject_qty);
