                        ...
                        <div class ="table-responsive">
                            <table class="table table-sm m-table table-bordered borderless">
                                ...
                                <tr v-for="(details, index) in form.details">
                                    ...
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input" v-model="details.current_qty" @input="totalTransQtyPriceCalc()" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.current_qty']">{{ errors['details.'+index+'.current_qty'][0] }}</div>
                                    </td>
                                    <td style="width: 160px;">
                                        <input style="width: 160px;" type="number" step="any" class="form-control form-control-sm m-input" v-model="details.receive_qty" @input="totalTransQtyPriceCalc()" placeholder="" readonly>
                                        <div class="requiredField" v-if="errors['details.'+index+'.receive_qty']">{{ errors['details.'+index+'.receive_qty'][0] }}</div>
                                    </td>
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input" v-model="details.adj_qty" @input="totalTransQtyPriceCalc()" placeholder="">
                                    </td>
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input" v-model="details.unit_price" @input="totalTransQtyPriceCalc()" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.unit_price']">{{ errors['details.'+index+'.unit_price'][0] }}</div>
                                    </td>
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input" v-model="details.trans_to_finishing" @input="totalTransQtyPriceCalc()" placeholder="">
                                    ...
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input" v-model="details.total_price" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.total_price']">{{ errors['details.'+index+'.total_price'][0] }}</div>
                                    </td>
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input" v-model="details.reject_qty" @input="totalTransQtyPriceCalc()" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.reject_qty']">{{ errors['details.'+index+'.reject_qty'][0] }}</div>
                                    </td>
                                    <td style="width: 160px;">
                                        <input style="width: 160px;" type="number" step="any" class="form-control form-control-sm m-input" v-model="details.remain_qty" placeholder="" readonly>
                                    </td>
                ...
                <div class="m-form__actions m-form__actions--solid">
                    <div class="row">
                        <div class="col-lg-12 m--align-right">
                            <button type="submit" v-if="permissions.indexOf('testing-section.approve') != -1" class="btn btn-sm btn-primary"  @click.prevent="store(1)" ><i class="la la-check"></i> <span>Approve</span></button>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            store:function(app){
                ...
                _this.form.approve_status = app;
                 _this.$loading(true);
                axios.post(base_url+'testing-section/store', this.form).then( (response) => {
                     _this.$loading(false);
            ...
            testingNoGenerate(){
                var _this = this;
                axios.get(base_url+'testing-no')
                    .then( (response) => {
                        _this.form.testing_no = response.data;
                    });
            },
        ...
        created(){
            var _this = this;
            _this.totalTransQtyPriceCalc();
            _this.testingNoGenerate();
            _this.form.testing_date = moment().format('DD/MM/YYYY');
