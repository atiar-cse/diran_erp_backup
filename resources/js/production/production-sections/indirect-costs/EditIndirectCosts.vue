        ...
        <form method="POST" enctype="multipart/form-data" v-on:submit.prevent="update(form.id)" id="editComponent" class="m-form m-form--fit m-form--label-align-right" >
            ...
            <div class="m-portlet__body">
                <div class="form-group m-form__group row">
                    <label for="indir_no" class="col-lg-2 col-form-label">#No <span class="requiredField">*</span></label>
                    <div class="col-lg-4">
                        <input type="text" id="indir_no" v-model="form.indir_no" class="form-control form-control-sm m-input" placeholder="Enter S/L" readonly>
                        <div class="requiredField" v-if="(errors.hasOwnProperty('indir_no'))">{{ (errors.hasOwnProperty('indir_no')) ? errors.indir_no[0] :'' }}</div>
                    </div>
                    <label for="m_datepicker_2" class="col-lg-2 col-form-label">Date <span class="requiredField">*</span></label>
                    <div class="col-lg-4">
                        <div class="input-group date">
                            <input type="text" class="form-control form-control-sm m-input datepicker" v-model="form.date" data-dateField="forming_date" readonly placeholder="Select date from picker" id="m_datepicker_2" autocomplete="off"/>
                        ...
                        <div class ="table-responsive">
                            <table class="table table-sm m-table table-bordered borderless">
                                ...
                                <tr v-for="(details, index) in form.details">
                                    ...
                                    <td class="tdWidth">
                                        <select class="form-control select2" v-bind:data-index="index" v-model="details.prod_indir_costs_type_id" style="width:100%">
                                            <option v-for="(value,index) in indir_costs_types" 
                                                :value="value.id"
                                            > {{value.indirect_cost_type}}
                                            </option>
                                        </select>
                                        <div class="requiredField" v-if="errors['details.'+index+'.prod_indir_costs_type_id']">{{ errors['details.'+index+'.prod_indir_costs_type_id'][0] }}</div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input" v-model="details.amount" placeholder="" @input="calcTotal()">
                                        <div class="requiredField" v-if="errors['details.'+index+'.amount']">{{ errors['details.'+index+'.amount'][0] }}</div>
                                    </td>                                   
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" step="any" v-model="form.total_amount" class="form-control form-control-sm m-input" placeholder="Total" readonly>
                                    </td>
                                    <td></td>
                                </tr>
                                </tbody>
                ...
                <div class="m-form__actions m-form__actions--solid">
                    <div class="row">
                        <div class="col-lg-12 m--align-right">
                            <button type="submit" v-if="permissions.indexOf('indirect-costs.approve') != -1" class="btn btn-sm btn-primary" @click.prevent="update(form.id,1)" ><i class="la la-check"></i> <span>Approve</span></button>
...
<script>
    ...
    export default {
        ...
        props:['permissions','editId','indir_costs_types'],
        ...
        methods:{
            ...
            edit(id) {
                var _this = this;                
                axios.get(base_url+'indirect-costs/'+id+'/edit')
                    .then( (response) => {
            ...
            update(id,app){
                ...
                axios.put(base_url+'indirect-costs/'+id, _this.form).then( (response) => {
