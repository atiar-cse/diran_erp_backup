<template>
    <div>
        ...
        <AddIndirectCosts
                v-else-if="add_form"
                :indir_costs_types="indir_costs_types"
...
<script>
    ...
    import AddIndirectCosts from './AddIndirectCosts.vue';
    import EditIndirectCosts from './EditIndirectCosts.vue';
    import ViewIndirectCosts from './ViewIndirectCosts.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'indirect-costs',
                options: {
                    ajax: {
                        url: base_url+"indirect-costs",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID'},
                    indir_no: { label: '#NO', sortable: true, searchable: true },
                    date: { label: 'Date', defaultOrder: 'DESC'  },
                    total_amount: { label: 'Total Amount',template:'<span class="pull-right"> {{row.total_amount}} </span>' },
                    ...
                    'production_indirect_costs.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('indirect-costs.edit') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permissions.indexOf('indirect-costs.destroy') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permissions.indexOf('indirect-costs.show') != -1 ) {
        ...
        methods: {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){                        
                        axios.delete(base_url+'indirect-costs/'+id).then((response) => {
