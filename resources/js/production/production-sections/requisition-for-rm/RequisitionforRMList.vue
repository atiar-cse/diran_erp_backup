...
<script>
    ...
    export default {
        ...
        props:['product_lists','permissions','warehouse_lists','account_lists','cost_center_lists'],
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'requisition-for-raw-material',
                options: {
                    ajax: {
                        url: base_url+"requisition-for-raw-material",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID' },
                    rm_requisition_no: { label: 'Requisition NO', sortable: true, searchable: true, defaultOrder: 'DESC'  },
                    requisition_date: { label: 'Requisition Date', sortable: true, searchable: true  },
                    requisition_types: { label: 'Requisition Type', sortable: true, searchable: true  },
                    'purchase_ware_houses.purchase_ware_houses_name': { label: 'Warehouse', template: '{{ row.purchase_ware_houses_name }}'  },
                    total_qty: { label: 'Total Qty'  },
                    total_amount: { label: 'Total Price'  },
                    ...
                    'production_requisition_for_rms.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>' },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('requisition-for-raw-material.edit') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permissions.indexOf('requisition-for-raw-material.destroy') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permissions.indexOf('requisition-for-raw-material.show') != -1 ) {
                ...
                deleteData(id,tr){
                    ...
                    swal({
                        ...
                        function(){
                            ...
                            axios.delete(base_url+'requisition-for-raw-material/'+id+'/delete').then((response) => {
