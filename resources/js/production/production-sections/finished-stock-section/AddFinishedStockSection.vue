        ...
        <form method="POST" enctype="multipart/form-data" v-on:submit.prevent="store" id="addComponent"
            ...
            <div class="m-portlet__body">
                <div class="form-group m-form__group row">
                    <label for="forming_no" class="col-lg-2 col-form-label">Finished Stock No <span
                        class="requiredField">*</span></label>
                    <div class="col-lg-4">
                        <input type="text" id="forming_no" v-model="form.finished_stock_no"
                               class="form-control form-control-sm m-input" placeholder="Enter Testing No" readonly>
                        <div class="requiredField" v-if="(errors.hasOwnProperty('finished_stock_no'))">{{
                            (errors.hasOwnProperty('finished_stock_no')) ? errors.finished_stock_no[0] :'' }}
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="input-group date">
                            <input type="text" class="form-control form-control-sm m-input datepicker"
                                   v-model="form.date" data-dateField="date" readonly
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm m-table table-bordered borderless">
                                ...
                                <tr>
                                    ...
                                    <th>Rec.F Testing Qty <span class="requiredField">*</span></th>
                                    <th>Adj. Qty</th>
                                    <th>Unit Price <span class="requiredField">*</span></th>
                                    <th>Trans to Assembling Qty <span class="requiredField">*</span></th>
                                ...
                                <tr v-for="(details, index) in form.details">
                                    ...
                                    <td class="tdWidth">
                                        <select class="form-control select2 select-product" v-bind:data-index="index"
                                            ...
                                            <option v-for="(value,index) in product_lists"
                                                    ...
                                                    :testing-opening-qty="value.finished_stk_current_qty"
                                                    :testing-receive-qty="value.finished_stk_receive_qty"
                                            > {{value.product_name}}
                                            </option>
                                        </select>
                                    ...
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input"
                                               v-model="details.adj_qty" @input="totalTransQtyPriceCalc()"
                                               placeholder="">
                                    </td>
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input"
                                    ...
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input"
                                               v-model="details.trans_to_store_qty" @input="totalTransQtyPriceCalc()"
                                               placeholder="">
                                        <div class="requiredField"
                                             v-if="errors['details.'+index+'.trans_to_store_qty']">{{
                                            errors['details.'+index+'.trans_to_store_qty'][0] }}
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input"
                                               v-model="details.reject_qty" @input="totalTransQtyPriceCalc()"
                                               placeholder="">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" v-model="form.total_adj_qty"
                                               class="form-control form-control-sm m-input" placeholder="Tot. Adj. Qty"
                                               readonly>
                                    </td>
                                    <td></td>
                                    <td>
                                        <input type="text" v-model="form.total_trans_to_store_qty"
                                               class="form-control form-control-sm m-input"
                                    ...
                                    <td>
                                        <input type="text" v-model="form.total_reject_qty"
                                               class="form-control form-control-sm m-input"
                                               placeholder="Total Reject Qty" readonly>
                                    </td>
                                    <td>
                ...
                <div class="m-form__actions m-form__actions--solid">
                    <div class="row">
                        <div class="col-lg-12 m--align-right">
                            <button type="submit" v-if="permissions.indexOf('finished-stock-section.approve') != -1"
    ...
    export default {
        ...
        methods: {
            ...
            store: function (app) {
                ...
                _this.$loading(true);
                axios.post(base_url + 'finished-stock-section', _this.form)
                    .then((response) => {
                        _this.$loading(false);
            ...
            totalTransQtyPriceCalc() {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    total_trans_to_store_qty = total_trans_to_store_qty + Number(details[i].trans_to_store_qty);

                    // Over head calculation
                    overhead_price = Number(details[i].trans_to_store_qty) * _this.overhead_kg_price;
                    _this.form.details[i].overhead_price = overhead_price.toFixed(2);

                    totalPrice = Number(details[i].unit_price) * Number(details[i].trans_to_store_qty) + overhead_price;
                    _this.form.details[i].total_price = totalPrice;

                    total_amount = totalPrice + total_amount;

                    remain_qty = Number(details[i].current_qty) + Number(details[i].receive_qty) + Number(details[i].adj_qty) - Number(details[i].trans_to_store_qty) - Number(details[i].reject_qty);
            ...
            finishedStockNoGenerate() {
                var _this = this;
                var token_no_geneate = 'finished_stock_no_generate';
                _this.$loading(true);
                axios.get(base_url + "finished-stock-section/?token_no_geneate=" + token_no_geneate)
                    .then((response) => {
                        _this.$loading(false);
                        _this.form.finished_stock_no = response.data;
                    });
            },

            getTestingLastPrice(product_id, dataIndex) {
                var _this = this;
                _this.$loading(true);
                axios.get(base_url + 'testing-section?token_last_price=yes&product_id=' + product_id)
                    .then((response) => {
                        _this.$loading(false);
            ...
            $('#addComponent').on('change', '.select-product', function (e) {
                ...
                if (kiln_weight > 0) {
                    ...
                    if (isDuplicated) {
                        ...
                        _this.getTestingLastPrice(selectedItem.val(), dataIndex);
                    }
                } else {
        ...
        created() {
            var _this = this;
            _this.totalTransQtyPriceCalc();
            _this.finishedStockNoGenerate();

            setTimeout(function () {
                _this.getOverheadCosting();
            }, 150);

            _this.form.date = moment().format('DD/MM/YYYY');
        }
    }
</script>
