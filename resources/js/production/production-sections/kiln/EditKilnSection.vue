        ...
        <form method="POST" enctype="multipart/form-data" v-on:submit.prevent="update(form.id)" id="editComponent" class="m-form m-form--fit m-form--label-align-right" >
            <div class="m-portlet__body">
                <div class="form-group m-form__group row">
                    ...
                    <div class="col-lg-4">
                        <input type="text" id="forming_no" v-model="form.kiln_no" class="form-control form-control-sm m-input" placeholder="Enter Kiln No" readonly>
                        <div class="requiredField" v-if="(errors.hasOwnProperty('kiln_no'))">{{ (errors.hasOwnProperty('kiln_no')) ? errors.kiln_no[0] :'' }}</div>
                    </div>
                    <label for="m_datepicker_2" class="col-lg-2 col-form-label">Kiln Date <span class="requiredField">*</span></label>
                    <div class="col-lg-4">
                        <div class="input-group date">
                            <input type="text" class="form-control form-control-sm m-input datepicker" v-model="form.kiln_date" data-dateField="kiln_date" readonly placeholder="Select date from picker" id="m_datepicker_2" autocomplete="off"/>
                        ...
                        <div class="requiredField" v-if="(errors.hasOwnProperty('kiln_date'))">{{ (errors.hasOwnProperty('kiln_date')) ? errors.kiln_date[0] :'' }}</div>
                    </div>
                </div>
                <div class="form-group m-form__group row">
                        ...
                        <div class ="table-responsive">
                            <table class="table table-sm m-table table-bordered borderless">
                                ...
                                <tr v-for="(details, index) in form.details">
                                    ...
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input width-80" v-model="details.kiln_weight" @input="totalTransQtyWeightCalc()" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.kiln_weight']">{{ errors['details.'+index+'.kiln_weight'][0] }}</div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input width-80" v-model="details.trans_to_hv_qty" @input="totalTransQtyWeightCalc()" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.trans_to_hv_qty']">{{ errors['details.'+index+'.trans_to_hv_qty'][0] }}</div>
                                    </td>
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input width-80" v-model="details.trans_to_hv_weight" input="totalTransQtyWeightCalc()" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.trans_to_hv_weight']">{{ errors['details.'+index+'.trans_to_hv_weight'][0] }}</div>
                                    </td>                                    
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input width-80" v-model="details.reject_qty" input="totalTransQtyWeightCalc()" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.reject_qty']">{{ errors['details.'+index+'.reject_qty'][0] }}</div>
                                    </td>
                                    <td>
                                        <input type="number" step="any" class="form-control form-control-sm m-input width-80" v-model="details.reject_weight" input="totalTransQtyWeightCalc()" placeholder="">
                                        <div class="requiredField" v-if="errors['details.'+index+'.reject_weight']">{{ errors['details.'+index+'.reject_weight'][0] }}</div>
                                    </td>
                                    <td class="width-160">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" v-model="form.total_trans_to_hv_qty" class="form-control form-control-sm m-input" placeholder="Total Testing Qty">
                                    </td>
                                    <td>
                                        <input type="text" v-model="form.total_trans_weight" class="form-control form-control-sm m-input" placeholder="Total Testing Weight">
                                    </td>                                     
                                    <td>
                                        <input type="text" v-model="form.total_reject_qty" class="form-control form-control-sm m-input" placeholder="Waste Qty">
                                    </td>
                                    <td>
                                        <input type="text" v-model="form.total_reject_weight" class="form-control form-control-sm m-input" placeholder="Waste Weight">
                                    </td>
                                    <td>
                ...
                <div class="m-form__actions m-form__actions--solid">
                    <div class="row">
                        <div class="col-lg-12 m--align-right">
                            <button type="submit" v-if="permissions.indexOf('kiln-section.approve') != -1" class="btn btn-sm btn-primary" @click.prevent="update(form.id,1)" ><i class="la la-check"></i> <span>Approve</span></button>
    ...
    export default {
        ...
        methods:{
            ...
            update(id,app){
                ...
                _this.$loading(true); 
                axios.put(base_url+'kiln-section/'+id, this.form).then( (response) => {
                    _this.$loading(false);
            ...
            totalTransQtyWeightCalc(){
                ...
                for(let i = 0; i < length; i++) {
                    ...
                    _this.form.details[i].trans_to_hv_weight = total_trans_weight.toFixed(2);
                    ...
                    overhead_price = Number(details[i].kiln_weight) * Number(details[i].trans_to_hv_qty) * _this.overhead_kg_price;
            ...
            getGlazeLastPrice(product_id,dataIndex) {
                var _this = this;
                axios.get(base_url + 'glaze-section?token_last_price=yes&product_id=' + product_id)
                    .then((response) => {

                        _this.form.details[dataIndex].unit_price = response.data.unit_price;
                        _this.form.details[dataIndex].kiln_weight = response.data.kiln_weight;
                        _this.totalTransQtyWeightCalc();
                    });
            },
        ...
        mounted(){
            ...
            $('#editComponent').on('change', '.select-product', function (e) {
                ...
                if (isDuplicated) {
                    ...
                    _this.getGlazeLastPrice( selectedItem.val(), dataIndex);
                }                
            });
