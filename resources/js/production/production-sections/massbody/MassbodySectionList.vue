<template>
    <div>
        ...
        <AddMassbodySection
                v-else-if="add_form"
                :rm_requisition_list="rm_requisition_list"
                :recycle_chip="recycle_chip"
...
<script>
    ...
    import AddMassbodySection from './AddMassbodySection.vue';
    import EditMassbodySection from './EditMassbodySection.vue';
    import ViewMassbodySection from './ViewMassbodySection.vue';
    ...
    export default {
        ...
        props:['rm_requisition_list','recycle_chip','permissions'],
        ...
        return {
            ...
            //VdtnetTable Options
            tableId: 'massbody-section',
            options: {
                ajax: {
                    url: base_url+"massbody-section",
                    dataSrc: (json) => {
                        return json.data
                    }
                },
            ...
            fields: {
                ...
                'production_massbodies.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                ...
                actions: {
                    ...
                    render: (data,type,row,meta) => {
                        var htmlButtons = '';
                        if (this.permissions.indexOf('massbody-section.edit') != -1 && row.approve_status !=1) {
                        ...
                        if (this.permissions.indexOf('massbody-section.destroy') != -1 && row.approve_status !=1) {
                        ...
                        if (this.permissions.indexOf('massbody-section.show') != -1 ) {
        ...
        deleteData(id,tr){
            ...
            swal({
                ...
                function(){
                    ...
                    axios.delete(base_url+'massbody-section/'+id+'/delete').then((response) => {
