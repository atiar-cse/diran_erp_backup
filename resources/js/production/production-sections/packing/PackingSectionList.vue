<template>
    <div>
        ...
        <AddPackingSection
                v-else-if="add_form"
                :finish_product_lists="finish_product_lists"
...
<script>
    ...
    import AddPackingSection from './AddPackingSection.vue';
    import EditPackingSection from './EditPackingSection.vue';
    import ViewPackingSection from './ViewPackingSection.vue';
    ...
    export default {
        ...
        props:['finish_product_lists', 'product_lists', 'permissions','warehouse_lists'],
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'packing-section',
                options: {
                    ajax: {
                        url: base_url+"packing-section",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    ...
                    trans_to_store_qty: { label: 'Trans to inventory Qty', sortable: false, searchable: false  },
                    total_price: { label: 'Amount', sortable: false, searchable: false  },
                    ...
                    'production_packings.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('packing-section.edit') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permissions.indexOf('packing-section.destroy') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permissions.indexOf('packing-section.show') != -1 ) {
        ...
        methods: {
            /*index(pageNo,perPage){
                ...
                axios.get(base_url+"packing-section/?page="+pageNo+"&perPage="+perPage).then((response) => {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'packing-section/'+id+'/delete').then((response) => {
