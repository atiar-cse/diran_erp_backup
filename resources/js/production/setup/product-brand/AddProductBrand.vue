            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="">Add Product Brand</h5>
                    ...
                    <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="store" class="m-form m-form--fit m-form--label-align-right">
                        <div class="modal-body">
                            <div class="form-group m-form__group row m--margin-top-20" :class="{'has-danger': (errors.hasOwnProperty('product_brand_name'))}">
                                <label class="col-form-label col-lg-4 col-sm-12">Product Brand Name <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input type="text" class="form-control" v-model="form.product_brand_name" placeholder="Enter product brand name" />
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('product_brand_name'))">{{ (errors.hasOwnProperty('product_brand_name')) ? errors.product_brand_name[0] :'' }}</div>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            store:function(){
                axios.post(base_url+'production-brand/store', this.form).then( (response) => {
                    $('#addModel').modal('hide');
            ...
            update:function (id) {
                axios.put(base_url+'production-brand/'+id+'/update', this.form).then( (response) => {
        ...
        mounted(){
           ...
           $('#addModel,#editModel').on('hidden.bs.modal', function(){
               _this.errors = {};
               _this.form = {'product_brand_name':'','description':'','status':'1'};
           });

            EventBus.$on('update-button-clicked', (id) => {
                _this.errors = {};
                axios.get(base_url+'production-brand/'+id+'/edit').then((response) => {
