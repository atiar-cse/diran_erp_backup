    ...
    <div>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="">Add Measure Unit</h5>
                    ...
                    <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="store" class="m-form m-form--fit m-form--label-align-right">
                        <div class="modal-body">
                            <div class="form-group m-form__group row m--margin-top-20" :class="{'has-danger': (errors.hasOwnProperty('product_category_name'))}">
                                <label class="col-form-label col-lg-4 col-sm-12">Measure Unit<span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input type="text" class="form-control" v-model="form.measure_unit" placeholder="Enter product measure unit" />
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('measure_unit'))">{{ (errors.hasOwnProperty('measure_unit')) ? errors.measure_unit[0] :'' }}</div>
                                </div>
                            </div>
        ...
        <div class="modal fade" id="editModel" role="dialog" aria-labelledby="" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit Measure Unit</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true" class="la la-remove"></span>
                        </button>
                    </div>
                    <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="update(form.id)" class="m-form m-form--fit m-form--label-align-right">
                        <div class="modal-body">
                            <div class="form-group m-form__group row m--margin-top-20" :class="{'has-danger': (errors.hasOwnProperty('product_category_name'))}">
                                <label class="col-form-label col-lg-4 col-sm-12">Measure Unit <span class="requiredField">*</span></label>
                                <div class="col-lg-8 col-md-8 col-sm-12">
                                    <input type="text" class="form-control" v-model="form.measure_unit" placeholder="Enter measure unit" />
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('measure_unit'))">{{ (errors.hasOwnProperty('measure_unit')) ? errors.measure_unit[0] :'' }}</div>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            store:function(){
                axios.post(base_url+'measure-unit/store', this.form).then( (response) => {
                    $('#addModel').modal('hide');
            ...
            update:function (id) {
                axios.put(base_url+'measure-unit/'+id+'/update', this.form).then( (response) => {
        ...
        mounted(){
           ...
           $('#addModel,#editModel').on('hidden.bs.modal', function(){
            ...
            EventBus.$on('update-button-clicked', (id) => {
                _this.errors = {};
                axios.get(base_url+'measure-unit/'+id+'/edit').then((response) => {
