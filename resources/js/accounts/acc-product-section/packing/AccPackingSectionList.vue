<template>
    <div>
        ...
        <AccEditPackingSection
                v-else-if="edit_form"
                :edit-id="edit_id"
                :permissions="permissions"
                :account_lists="account_lists"
        ></AccEditPackingSection>
        <AccViewPackingSection
                v-else-if="view_form"
                :edit-id="edit_id"
        ></AccViewPackingSection>
...
<script>
    ...
    import AccEditPackingSection from './AccEditPackingSection.vue';
    import AccViewPackingSection from './AccViewPackingSection.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'acc-packing-voucher',
                options: {
                    ajax: {
                        url: base_url+"acc-packing-voucher",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID'},
                    packing_no: { label: 'Packing NO', sortable: true, searchable: true, defaultOrder: 'DESC'  },
                    packing_date: { label: 'Packing Date', sortable: true, searchable: true  },
                    'production_products.product_name': { label: 'Finishing Product',template:'{{ row.product_name }}', sortable: true, searchable: true  },
                    narration: { label: 'Narration', sortable: false, searchable: false  },
                    total_rm_qty: { label: 'Used Qty', sortable: false, searchable: false  },
                    total_rm_price: { label: 'Amount', sortable: false, searchable: false  },
                    account_status: { label: 'Accounts Approval',template:' <div v-if="row.account_status == 0"  class="badge badge-pill badge-danger" >Pending</div><div v-if="row.account_status == 1"  class="badge badge-pill badge-success" >Approved</div>'},
                    'production_packings.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.account_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('acc-packing-voucher.edit') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-packing-voucher.destroy') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-packing-voucher.show') != -1 ) {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'acc-packing-voucher/'+id+'/delete').then((response) => {
