<template>
    <div>
        ...
        <AccEditFormingSection
                v-else-if="edit_form"
                :edit-id="edit_id"
                :account_lists="account_lists"
                :permissions="permissions"
        ></AccEditFormingSection>
        <AccViewformingSection
                v-else-if="view_form"
                :edit-id="edit_id"
        ></AccViewformingSection>
...
<script>
    ...
    import AccEditFormingSection from './AccEditFormingSection.vue';
    import AccViewformingSection from './AccViewformingSection.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'acc-forming-voucher',
                options: {
                    ajax: {
                        url: base_url+"acc-forming-voucher",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    ...
                    'production_formings.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.account_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('acc-forming-voucher.edit') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-forming-voucher.destroy') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-forming-voucher.show') != -1 ) {
