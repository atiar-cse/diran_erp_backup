<template>
    <div>
        ...
        <AccEditRequisitionforRM
                v-else-if="edit_form"
                :edit-id="edit_id"
                :permissions="permissions"
                :account_lists="account_lists"
        ></AccEditRequisitionforRM>
        <AccViewRequisitionForRm
                v-else-if="view_form"
                :edit-id="edit_id"
        ></AccViewRequisitionForRm>
...
<script>
    ...
    import AccEditRequisitionforRM from './AccEditRequisitionforRM.vue';
    import AccViewRequisitionForRm from './AccViewRequisitionForRm.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'acc-req-rm-voucher',
                options: {
                    ajax: {
                        url: base_url+"acc-req-rm-voucher",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    ...
                    estimate_qty_for_production: { label: 'Production Qty'  },
                    ...
                    'production_requisition_for_rms.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.account_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('acc-req-rm-voucher.edit') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-req-rm-voucher.show') != -1 ) {
