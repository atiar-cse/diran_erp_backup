<template>
    <div>
        ...
        <AccEditFinishedStockSection
                v-else-if="edit_form"
                :edit-id="edit_id"
                :account_lists="account_lists"
                :permissions="permissions"
        ></AccEditFinishedStockSection>
        <AccViewFinishedStockSection
                v-else-if="view_form"
                :edit-id="edit_id"
        ></AccViewFinishedStockSection>
...
<script>
    ...
    import AccEditFinishedStockSection from './AccEditFinishedStockSection.vue';
    import AccViewFinishedStockSection from './AccViewFinishedStockSection.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'acc-finish-stock-voucher',
                options: {
                    ajax: {
                        url: base_url+"acc-finish-stock-voucher",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID' },
                    finished_stock_no: { label: 'Finish Stock NO', sortable: true, searchable: true, defaultOrder: 'DESC'  },
                    date: { label: 'Date', sortable: true, searchable: true  },
                    ...
                    'production_finished_stocks.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.account_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('acc-finish-stock-voucher.edit') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-finish-stock-voucher.destroy') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-finish-stock-voucher.show') != -1 ) {
