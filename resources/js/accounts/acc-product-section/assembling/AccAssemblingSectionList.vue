<template>
    <div>
        ...
        <AccEditAssemblingSection
                v-else-if="edit_form"
                :edit-id="edit_id"
                :account_lists="account_lists"
                :permissions="permissions"
        ></AccEditAssemblingSection>
        <AccViewAssemblingSection
                v-else-if="view_form"
                :edit-id="edit_id"
        ></AccViewAssemblingSection>
...
<script>
    ...
    import AccEditAssemblingSection from './AccEditAssemblingSection.vue';
    import AccViewAssemblingSection from './AccViewAssemblingSection.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'acc-assembly-voucher',
                options: {
                    ajax: {
                        url: base_url+"acc-assembly-voucher",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID' },
                    assembling_no: { label: 'Assembling NO', sortable: true, searchable: true, defaultOrder: 'DESC'  },
                    assembling_date: { label: 'Assembling Date', sortable: true, searchable: true  },
                    'production_products.product_name': { label: 'Finishing Product',template:'{{ row.product_name }}', sortable: true, searchable: true  },
                    trans_to_packing_qty: { label: 'Trans to Store Qty' },
                    ...
                    'production_assemblings.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.account_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('acc-assembly-voucher.edit') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-assembly-voucher.destroy') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-assembly-voucher.show') != -1 ) {
