<template>
    <div>
        ...
        <AccEditShappingSection
                v-else-if="edit_form"
                :edit-id="edit_id"
                :account_lists="account_lists"
                :permissions="permissions"
        ></AccEditShappingSection>
        <AccViewShappingSection
                v-else-if="view_form"
                :edit-id="edit_id"
        ></AccViewShappingSection>
...
<script>
    ...
    import AccEditShappingSection from './AccEditShappingSection.vue';
    import AccViewShappingSection from './AccViewShappingSection.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'acc-shapping-voucher',
                options: {
                    ajax: {
                        url: base_url+"acc-shapping-voucher",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    ...
                    shapping_date: { label: 'Shapping Date', sortable: true, searchable: true  },
                    ...
                    'production_shappings.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.account_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('acc-shapping-voucher.edit') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-shapping-voucher.destroy') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-shapping-voucher.show') != -1 ) {
