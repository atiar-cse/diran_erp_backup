<template>
    <div>
        ...
        <div class="m-portlet__body" v-if="fixed_asset_dep_list">
            <div class="row">
        ...
        <AddFixedAssetDepreciation
            v-else-if="add_form"
            :sub_code_lists="sub_code_lists"
            :permissions="permissions"
        ></AddFixedAssetDepreciation>
        <EditFixedAssetDepreciation v-else-if="edit_form"
                                    :sub_code_lists="sub_code_lists"
                                    :permissions="permissions"
                                    :edit-id="edit_id"
        ></EditFixedAssetDepreciation>
        <ViewFixedAssetDepreciation
            v-else-if="view_form"
            :edit-id="edit_id"
        ></ViewFixedAssetDepreciation>
    </div>
...
<script>
    ...
    import AddFixedAssetDepreciation from './AddFixedAssetDepreciation.vue';
    import EditFixedAssetDepreciation from './EditFixedAssetDepreciation.vue';
    import ViewFixedAssetDepreciation from './ViewFixedAssetDepreciation.vue';
    ...
    export default {
        ...
        props: ['permissions', 'sub_code_lists'],

        data: function () {
            return {
                fixed_asset_dep_list: true,
                add_form: false,
                edit_form: false,
                edit_id: false,
                view_form: false,
                ...
                //VdtnetTable Options
                tableId: 'fixed-asset-depreciation',
                options: {
                    ajax: {
                        url: base_url + "fixed-asset-depreciation",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: {label: 'ID'},
                    depreciations_date: {label: 'Date', sortable: true, searchable: true},
                    depreciations_no: {label: 'Serial', sortable: true, searchable: true, defaultOrder: 'DESC'},
                    'accounts_sub_codes.sub_code_title': {
                        label: 'Account Name',
                        template: '{{ row.sub_code_title }} ({{ row.sub_code }})',
                        sortable: true,
                        searchable: true
                    },
                    total_amount: {label: 'Total Amount'},
                    total_depreciations: {label: 'Depreciate Amount '},
                    total_current_amount: {label: 'Amount After Depreciat '},
                    approve_status: {
                        label: 'Approve State',
                    ...
                    actions: {
                        ...
                        render: (data, type, row, meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('fixed-asset-depreciation.edit') != -1 && row.approve_status != 1) {
                            ...
                            if (this.permissions.indexOf('fixed-asset-depreciation.destroy') != -1 && row.approve_status != 1) {
                            ...
                            if (this.permissions.indexOf('fixed-asset-depreciation.show') != -1) {
        ...
        methods: {
            ...
            editData(id) {
                ...
                _this.fixed_asset_dep_list = false;
            ...
            deleteData(id, tr) {
                ...
                swal({
                    ...
                    function () {
                        ...
                        _this.$loading(true);
                        axios.delete(base_url + 'fixed-asset-depreciation/' + id)
                            .then((response) => {
                                _this.$loading(false);
        ...
        created() {
            ...
            $('body').on('click', '#listButton', function () {
                _this.add_form = false;
                _this.fixed_asset_dep_list = true;
                _this.edit_form = false;
                _this.view_form = false;
                $('#addButton').show();
                $('#listButton').hide();
