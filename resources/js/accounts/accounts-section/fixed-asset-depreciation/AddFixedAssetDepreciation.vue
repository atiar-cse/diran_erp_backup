                            ...
                            <table class="table table-sm m-table table-bordered borderless">
                                ...
                                <tr v-for="(details, index) in form.details">
                                    ...
                                    <td>
                                        ...
                                        <div class="requiredField" v-if="errors['details.'+index+'.percentage']">{{
                                            errors['details.'+index+'.percentage'][0] }}
                                        </div>
                                    </td>
                                    <td>
                    ...
                    <div class="m-form__actions m-form__actions--solid">
                        <div class="row">
                            <div class="col-lg-12 m--align-right">
                                ...
                                <button id="submit" type="submit" class="btn btn-sm btn-info" @click.prevent="store(0)"><i
                                    class="la la-save"></i> <span>Save and Go List</span></button>
                                <button type="submit" id="approve" class="btn btn-sm btn-success" @click.prevent="store(1)"><i
...
<script>
    ...
    export default {
        ...
        props: ['sub_code_lists', 'permissions'],

        data: function () {
            return {

                fixed_asset_dep_list: false,
                add_form: true,
                edit_form: false,
        ...
        methods: {
            transactionNoGenerate() {
                var _this = this;
                _this.$loading(true);
                axios.get(base_url + 'sl-no')
                    .then((response) => {
                        _this.$loading(false);
                        _this.form.depreciations_no = response.data;
                    });
            },
            ...
            store(app) {
                ...
                _this.$loading(true);
                axios.post(base_url + 'fixed-asset-depreciation', _this.form)
                    .then((response) => {
                        _this.$loading(false);
            ...
            loadDetails() {
                var _this = this;
                var id = _this.form.sub_code_id;

                _this.$loading(true);
                axios.get(base_url + "fixed-asset-depreciation/" + id + "/chart-list")
                    .then((response) => {
                        _this.$loading(false);
            ...
            totalQtyAndPrice() {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    var percentage = Number(details[i].percentage);
