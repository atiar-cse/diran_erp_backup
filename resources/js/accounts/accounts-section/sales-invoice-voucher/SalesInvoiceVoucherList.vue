<template>
    <div>
        <div class="m-portlet__body" v-if="sales_voucher_list">
            <div class="row">
        ...
        <EditSalesInvoiceVoucher
            v-else-if="edit_form"
                :edit-id="edit_id"
                :permission="permission"
                :pid_lists="pid_lists"
                :inv_lists="inv_lists"
                :challan_lists="challan_lists"
...
<script>
    ...
    import EditSalesInvoiceVoucher from './EditSalesInvoiceVoucher.vue';
    import ViewSalesInvoiceVoucher from './ViewSalesInvoiceVoucher.vue';
    ...
    export default {
        ...
        data: function(){
            return {
              ...
              //VdtnetTable Options
                tableId: 'sales-invoice-voucher',
                options: {
                    ajax: {
                        url: base_url+"sales-invoice-voucher",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID' },
                    vouchers_no: { label: 'Invoice NO 4', sortable: true, searchable: true, defaultOrder: 'DESC'  },
                    'sales_delivery_challans.sales_delivery_no': { label: 'Challan NO', template: '{{ row.sales_delivery_no }}',sortable: true, searchable: true },
                    sales_date: { label: 'Sales Date',/*render:(data) =>{return this.dateFormat(data)},*/sortable: true, searchable: true  },
                    ...
                    'accounts_sales_invoice_vouchers.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    actions: {
                        isLocal: true,
                        label: 'Actions',
                        render: (data,type,row) => {
                            var htmlButtons = '';
                            if (this.permission.indexOf('sales-invoice-voucher.edit') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permission.indexOf('sales-invoice-voucher.destroy') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permission.indexOf('sales-invoice-voucher.show') != -1 && row.approve_status ==1) {
                            ...
                            if ((this.permission.indexOf('super-admin') != -1 || this.permissions.indexOf('sales-invoice-voucher.unapprove') != -1) && row.approve_status ==1 ) {
        ...
        methods: {
            ...
            editData(id){
                ...
                _this.sales_voucher_list = false;
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'sales-invoice-voucher/'+id+'/delete').then((response) => {
            ...
            doSuperAdmin(data){ //Unapprove
                ...
                swal({
                    ...
                    function(){
                        axios.get(base_url+'sales-invoice-voucher/'+id+'?isSuperAdmin=unapprove').then((response) => {
