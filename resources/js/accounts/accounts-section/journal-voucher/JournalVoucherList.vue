...
<script>
    ...
    import AddJournalVoucher from './AddJournalVoucher.vue';
    import EditJournalVoucher from './EditJournalVoucher.vue';
    import ViewJournalVoucher from './ViewJournalVoucher.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'journal-voucher',
                options: {
                    ajax: {
                        url: base_url+"journal-voucher",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID' },
                    //transaction_type: {label: 'type', visible: false, printable:  false, defaultOrder: 'ASC'},
                    transaction_reference_no: { label: 'Reference NO', sortable: true, searchable: true, defaultOrder: 'DESC'  },
                    transaction_date: { label: 'Date',sortable: true, searchable: true  },
                    total_debit: { label: 'Total Debit' },
                    ...
                    'accounts_journal_entries.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data, type, row, meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('journal-voucher.edit') != -1 && row.approve_status !=1 && row.transaction_type == 1) {
                            ...
                            if (this.permissions.indexOf('journal-voucher.destroy') != -1 && row.approve_status !=1 && row.transaction_type == 1) {
                            ...
                            if (this.permissions.indexOf('journal-voucher.show') != -1 ) {
                            ...
                            if ((this.permissions.indexOf('super-admin') != -1 || this.permissions.indexOf('journal-voucher.unapprove') != -1) && row.approve_status ==1 ) {
        ...
        methods: {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'journal-voucher/'+id).then((response) => {
            ...
            doSuperAdmin(data){ //Unapprove
                ...
                swal({
                    ...
                    function(){
                        axios.get(base_url+'journal-voucher/'+id+'?isSuperAdmin=unapprove').then((response) => {
