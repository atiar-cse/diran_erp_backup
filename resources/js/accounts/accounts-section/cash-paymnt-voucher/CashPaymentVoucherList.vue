...
<script>
    ...
    import AddCashPaymentVoucher from './AddCashPaymentVoucher.vue';
    import EditCashPaymentVoucher from './EditCashPaymentVoucher.vue';
    import ViewCashPaymentVoucher from './ViewCashPaymentVoucher.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'cash-payment-voucher',
                options: {
                    ajax: {
                        url: base_url+"cash-payment-voucher",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    ...
                    'accounts_cash_payment_vouchers.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data, type, row, meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('cash-payment-voucher.edit') != -1 && row.approve_status !=1 ) {
                            ...
                            if (this.permissions.indexOf('cash-payment-voucher.destroy') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permissions.indexOf('cash-payment-voucher.show') != -1) {
                            ...
                            if ((this.permissions.indexOf('super-admin') != -1 || this.permissions.indexOf('cash-payment-voucher.unapprove') != -1) && row.approve_status ==1 ) {
        ...
        methods: {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'cash-payment-voucher/'+id).then((response) => {
            ...
            doSuperAdmin(data){ //Unapprove                
                ...
                swal({
                    ...
                    function(){                        
                        axios.get(base_url+'cash-payment-voucher/'+id+'?isSuperAdmin=unapprove').then((response) => {
