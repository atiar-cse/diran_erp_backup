<template>
    <div>
        ...
        <AddBankBranch :bank_lists="bank_lists"></AddBankBranch>
    </div>
</template>
...
<script>
    ...
    import AddBankBranch from './AddBankBranch.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                fields: {
                    id: { label: 'ID' },
                    bank_branch_names: { label: 'Branch Name', sortable: true, searchable: true/*, defaultOrder: 'asc'*/  },
                    bank_branch_codes: { label: 'Branch Code' },
                    'accounts_bank_names.accounts_bank_names': { label: 'Bank',template:'{{ row.accounts_bank_names }}', sortable: true, searchable: true },
                    bank_branch_contact_no: { label: 'Contact No'  },
                    bank_branch_address: { label: 'Address' ,sortable: false, searchable: false},
                    'accounts_bank_branches.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span>', sortable: false, searchable: false },
                    status: {
                        label: 'Status',
                        render: (data) => {
                            if (data==1) {
                    ...
                    actions: {
                        ...
                        render: (data) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('bank-branch.edit') != -1) {
                            ...
                            if (this.permissions.indexOf('bank-branch.destroy') != -1) {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'bank-branch/'+id+'/delete').then((response) => {
