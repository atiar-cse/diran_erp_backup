<template>
    <div>
                ...
                <div class="row align-items-center">
                    <div class="col-xl-12 order-2 order-xl-1">
                        <div class="form-group m-form__group row align-items-center">
                            ...
                            <div class="col-md-3">
                                <div class="">
                                    <label id="coa_id" class="m-label m-label--single">COA :</label>
                                    <div class="m-form__control">
                                        <select class="form-control select2" id="coa_id" data-selectField="coa_id"  v-model="form.coa_id"  style="width:100%">
                                            <option v-for="(svalue,sindex) in bank_coa_list" :value="svalue.id" > 
        ...
        <div class="m-invoice__head print_only">
            <div class="m-invoice__container m-invoice__container--centered">
                <div class="row invoice-header"  style="padding-top: 0px;padding-bottom:15px;">
                    <div class="col-md-4">
                        <div class="db-data">
                            <p><span style="font-weight:600">Chart Of A/C Head :</span> <span style="font-size:10px;">{{coa_title}}</span></p>
                    ...
                    <div class="col-md-4 text-center">
                        <span class="m-invoice__desc">
                            <p><span style="font-weight:600">Bank Statement Report</span></p>
        ...
        <div class="m-portlet__body" v-if="bank_statement_report_list">
            <!--begin: Datatable -->
            <table class="table table-bordered table-hover" id="m_table_1">
                ...
                <tbody v-for="(jrnl,index) in resultData">
                    <tr v-for="(row,index2) in jrnl.details">
                        <td v-if="index2 ==0">{{format_date(jrnl.transaction_date)}}</td>
                        ...
                        <td v-else></td>
                        <td> {{ result(row.coa.opening_balance,row.debit_amount,row.credit_amount) }}</td>
                    </tr>
                </tbody>
...
<script>
    ...
    export default {
        ...
        data: function(){
            return {
                bank_statement_report_list:true,
        ...
        methods: {
            ...
            result(op,dr,cr)
            {
                return Number(op)+Number(cr)-Number(dr);
            },
            ...
            search: function () {
                ...
                var coa_id = _this.form.coa_id;
                var from_date = _this.form.from_date;
                var end_date = _this.form.end_date;

                $('#icon_submit').addClass('la-spin');
                axios.get(base_url+"bank-statement-report/?coa_id="+coa_id+"&from_date="+from_date+"&end_date="+end_date)
                    .then( (response) => {
            ...
            calcTotal(){
                ...
                _this.totalDebit = totalDr;
                _this.totalCredit = totalCr;
            },

            doExportPdf() {
                var _this = this;

                window.open(base_url + 'bank-statement-report?export=pdf&coa_id='+_this.form.coa_id+'&from_date='+ _this.form.from_date +'&end_date='+ _this.form.end_date +'', "_blank");
            },

            doExportExcel() {
                var _this = this;

                window.open(base_url + 'bank-statement-report?export=excel&coa_id='+_this.form.coa_id+'&from_date='+ _this.form.from_date +'&end_date='+ _this.form.end_date +'', "_blank");
            },
        },
        ...
        mounted(){
            ...
            $('#listComponent').on('change', '.select2', function (e) {
                ...
                _this.coa_title = $('option:selected', this).text();
            });            
        },
