...
<script>
    ...
    import AccEditLcInsurance from './AccEditLcInsurance.vue';
    import AccViewLcInsurance from './AccViewLcInsurance.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'acc-lc-insurance',
                options: {
                    ajax: {
                        url: base_url+"acc-lc-insurance",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    id: { label: 'ID' },
                    'lc_opening_sections.lc_no': { label: 'LC No',template:'{{ row.lc_no}}', sortable: true, searchable: true, defaultOrder: 'asc'  },
                    'lc_opening_sections.lc_total_value': { label: 'LC Value',template:'{{ row.lc_total_value}}', sortable: true, searchable: true, defaultOrder: 'asc'  },
                    insurance_company: { label: 'Insurance Company' },
                    insurance_date: { label: 'Date', sortable: true, searchable: true  },
                    insurance_no: { label: 'Insurance No.'  },
                    insurance_amount: { label: 'Amount'  },
                    approve_status: { label: 'Approve State',template:' <div v-if="row.approve_status == 0"   class="badge badge-pill badge-danger" >Processing</div><div v-if="row.approve_status == 1"  class="badge badge-pill badge-success" >Approve</div>' },
                    'lc_insurances.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ac_ed_name">Edited By  : {{ row.ac_ed_name }}</span><br><span v-if="row.account_status==1 && row.ac_ap_name">Approved By  : {{ row.ac_ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('acc-lc-insurance.edit') != -1 && row.account_status !=1) {
                            ...
                            if (this.permissions.indexOf('acc-lc-insurance.show') != -1 ) {
