<template>
    <div>
        ...
        <AccEditLATREntry v-else-if="edit_form"
...
<script>
    ...
    import AccEditLATREntry from './AccEditLATREntry.vue';
    import AccViewLATREntry from './AccViewLATREntry.vue';
    ...
    export default {
        ...
        props:['permissions','lc_lists','product_lists','measure_unit_lists','account_list'],
        ...
        data: function(){
            return {
                ...
                //VdtnetTable Options
                tableId: 'acc-latr-entry',
                options: {
                    ajax: {
                        url: base_url+"acc-latr-entry",
                        dataSrc: (json) => {
                            return json.data
                        }
                    },
                ...
                fields: {
                    ...
                    lc_margin_percentage: { label: 'LC Margin (%)', sortable: false, searchable: false},
                    total_qty: { label: 'Qty', sortable: false, searchable: false  },
                    total_amount: { label: 'Total Amount (BDT)',sortable: false, searchable: false },
                    latr_percentage: { label: 'LTR (%)',sortable: false, searchable: false },
                    latr_total_amount: { label: 'LTR Amount',sortable: false, searchable: false },
                    ...
                    'lc_latr_entries.created_by': { label: 'User', template: '<span v-if="row.created_by">Add By : {{ row.user_name }}</span><br><span v-if="row.updated_by">Edit By  : {{ row.user_name }}</span><br><span v-if="row.account_status==1">Approved By  : {{row.user_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            ...
                            if (this.permissions.indexOf('acc-latr-entry.edit') != -1 && row.account_status !=1 ) {
                            ...
                            if (this.permissions.indexOf('acc-latr-entry.show') != -1 ) {
