    ...
    <div>
        <div class="m-portlet__body" v-if="product_list">
            ...
            <div class="table-responsive">
                <vdtnet-table :id="tableId" ref="table" :fields="fields" :opts="options"
                              @edit="doAlertEdit"
...
<script>
    ...
    import AddSalesInvoice from './AddSalesInvoice.vue';
    import EditSalesInvoice from './EditSalesInvoice.vue';
    import ViewSalesInvoice from './ViewSalesInvoice.vue';
    import ViewSalesInvoiceWithoutPrice from './ViewSalesInvoiceWithoutPrice.vue';
    ...
    export default {
        ...
        data: function(){
            return {
                ...
                fields: {
                    id: { label: 'ID' },
                    sales_invoices_no: { label: 'Challan NO', sortable: true, searchable: true, defaultOrder: 'DESC'  },
                    sales_invoices_contract_no: { label: 'Contract NO',sortable: true, searchable: true  },
                    sales_invoices_date: { label: 'Sales Date',sortable: true, searchable: true  },
                    sales_invoices_type: { label: 'Type',sortable: true, searchable: true  },
                    ...
                    sales_invoices_narration: {
                        label: 'Narration',
                        template: '<span style="white-space: pre-line;">{{ row.sales_invoices_narration }}</span>'
                    },
                    sales_invoices_total_qty: { label: 'Total Qty' },
                    sales_invoices_total_price: { label: 'Total Price' },
                    ...
                    'sales_invoices.created_by': { label: 'User', template: '<span v-if="row.ad_name">Added By : {{ row.ad_name }}</span><br><span v-if="row.ed_name">Edited By  : {{ row.ed_name }}</span><br><span v-if="row.approve_status==1 && row.ap_name">Approved By  : {{ row.ap_name }} </span><br>', sortable: false, searchable: false },
                    ...
                    actions: {
                        ...
                        render: (data,type,row,meta) => {
                            var htmlButtons = '';
                            if (this.permissions.indexOf('sales-invoice.edit') != -1 && row.approve_status !=1) {
                            ...
                            if (this.permissions.indexOf('sales-invoice.destroy') != -1 && row.approve_status !=1) {
                            ...
                            if ((this.permissions.indexOf('super-admin') != -1 || this.permissions.indexOf('sales-invoice.unapprove') != -1) && row.approve_status ==1 ) {
        ...
        methods: {
            ...
            deleteData(id,tr){
                ...
                swal({
                    ...
                    function(){
                        ...
                        axios.delete(base_url+'sales-invoice/'+id+'/delete').then((response) => {
            ...
            doSuperAdmin(data){ //Unapprove
                ...
                swal({
                        ...
                        text: "It will be marked as Unapproved and Amounts will be removed from Accounts! The Stock Entry Quantity will be increased. And Regarding 'Sales invoice/Challan & Stock Entry' & 'Sales Invoice Voucher' information will be removed!",
                        type: "warning",
                        showCancelButton: true,
                        confirmButtonColor: "#DD6B55",
                    ...
                    function(){
                        axios.get(base_url+'sales-invoice/'+id+'?isSuperAdmin=unapprove').then((response) => {
                            if (response.data.status == 'success') {
                                swal("Unapproved!", response.data.message, "success");
                                _this.showMassage(response.data);
                                _this.doReload();
                            } else {
                                _this.showMassage(response.data);
            ...
            doAlertView2(data) {
                this.viewWithoutPriceData(data.id);
            },
