@extends('master')
@section('title','Manage Bill Payment Voucher ')
@section('content')
    <!-- BEGIN: Subheader -->
                            ...
                            <div class="m-portlet__head-tools">
                                <a href="javascript:void(0)"  class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" id="addButton">
                                    <span>
                                        ...
                                        <span>Add Bill Payment Voucher </span>
                                    </span>
                                </a>
                                <a href="javascript:void(0)" class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" style="display: none" id="listButton">
                                    <span>
                                        <i class="fa fa-list"></i>
                                        <span>Manage Bill Payment Voucher</span>
                                    </span>
                                </a>
                            </div>
                        </div>
                        <bill-paymnt-voucher-list
                                {{-- :product_lists="{{json_encode($productLists)}}"
                                 :po_order_generate="{{json_encode($PoOrderGenerate)}}"
                                 :supplier_lists="{{json_encode($supplierLists)}}"
                                 :requisition_lists="{{json_encode($requisitionLists)}}"--}}
                        ></bill-paymnt-voucher-list>
                        <!--begin::Form-->
                        <!--end::Form-->
                    </div>
...
@push('custom_scripts')
<script type="text/javascript" src="{!! asset('js/account/bill-paymnt-voucher.js') !!}"></script>
@endpush
