@extends('master')
@section('title','Manage Bank Receipt Voucher ')
@section('content')
    <!-- BEGIN: Subheader -->
                    ...
                    <div class="m-portlet">
                        <div class="m-portlet__head">
                            ...
                            <div class="m-portlet__head-tools">
                                @if (in_array('bank-receipt-voucher.store',permissionCheck()))
                                <a href="javascript:void(0)"  class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" id="addButton">
                                    <span>
                                        ...
                                        <span>Add Bank Receipt Voucher </span>
                                    </span>
                                </a>
                                @endif
                                <a href="javascript:void(0)" class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" style="display: none" id="listButton">
                                    <span>
                                        <i class="fa fa-list"></i>
                                        <span>Manage Bank Receipt Voucher</span>
                                    </span>
                                </a>
                            </div>
                        </div>
                        <bank-receipt-voucher-list
                                :permissions="{{ json_encode(permissionCheck()) }}"
                                :cost_center_list="{{json_encode($costCenterList)}}"
                                {{--:account_list="{{json_encode($chartofAccountList)}}"--}}
                                :subcode2_list="{{json_encode($subCode2List)}}"
...
@push('custom_scripts')
<script type="text/javascript" src="{!! asset('js/account/bank-receipt-voucher.js') !!}"></script>
@endpush
