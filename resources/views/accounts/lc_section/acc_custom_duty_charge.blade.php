@extends('master')
@section('title','Manage Custom Duty Charge Entry ')
@section('content')
    <!-- BEGIN: Subheader -->
                    ...
                    <ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
                        ...
                        <li class="m-nav__item">
                            <a href="" class="m-nav__link">
                                <span class="m-nav__link-text">LC Voucher</span>
                            </a>
                        </li>
                        <li class="m-nav__separator">-</li>
                        <li class="m-nav__item">
                            <a href="" class="m-nav__link">
                                <span class="m-nav__link-text">Accounts Section</span>
                            </a>
                        </li>
                        <li class="m-nav__separator">-</li>
                        <li class="m-nav__item">
                            ...
                            <div class="m-portlet__head-tools">
                                <a href="javascript:void(0)" class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" style="display: none" id="listButton">
                                    <span>
                                        ...
                                        <span>Manage Custom Duty Charge Entry</span>
                                    </span>
                                </a>
                            </div>
                        </div>
                        <acc-custom-duty-charge-entry-list
                                :permissions="{{ json_encode(permissionCheck()) }}"
                                :lc_lists="{{json_encode($lcLists)}}"
                                :lc_custom_duty_name_lists="{{json_encode($lcCustomDutyNameLists)}}"
                                :product_lists="{{json_encode($productLists)}}"
                                :account_list="{{json_encode($chartofAccountList)}}"
                                :supplier_list="{{json_encode($supplierList)}}"
                        ></acc-custom-duty-charge-entry-list>
                        <!--begin::Form-->
                        <!--end::Form-->
                    </div>
...
@push('custom_scripts')
<script type="text/javascript" src="{!! asset('js/account/acc-custom-duty-charge-entry.js') !!}"></script>
@endpush
