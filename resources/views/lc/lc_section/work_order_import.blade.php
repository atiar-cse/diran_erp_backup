@extends('master')
@section('title','Manage Work Order Import ')
@section('content')
    <!-- BEGIN: Subheader -->
                    ...
                    <div class="m-portlet">
                        <div class="m-portlet__head">
                            ...
                            <div class="m-portlet__head-tools">
                                ...
                                @if (in_array('work-order-import.create',permissionCheck()))
                                <a href="javascript:void(0)"  class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" id="addButton">
                                    <span>
                                        ...
                                        <span>Add Work Order Import </span>
                                    </span>
                                </a>
                                @endif
                                <a href="javascript:void(0)" class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" style="display: none" id="listButton">
                                    <span>
                                        <i class="fa fa-list"></i>
                                        <span>Manage Work Order Import</span>
                                    </span>
                                </a>
                            </div>
                        </div>
                        <work-order-import-list
                                 :permissions="{{ json_encode(permissionCheck()) }}"
                                 :product_lists="{{json_encode($productLists)}}"
                                 :supplier_lists="{{json_encode($supplierLists)}}"
                                 :currency_lists="{{json_encode($currecyLists)}}"
                                 :coa_lists="{{json_encode($coaLists)}}"
                        ></work-order-import-list>
                        <!--begin::Form-->
                        <!--end::Form-->
                    </div>
...
@push('custom_scripts')
<script type="text/javascript" src="{!! asset('js/lc/work-order-import.js') !!}"></script>
@endpush
