@extends('master')
@section('title','Manage Commercial Invoice ')
@section('content')
    <!-- BEGIN: Subheader -->
                    ...
                    <div class="m-portlet">
                        <div class="m-portlet__head">
                            ...
                            <div class="m-portlet__head-tools">
                                ...
                                @if (in_array('commercial-invoice-entry.create',permissionCheck()))
                                <a href="javascript:void(0)"  class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" id="addButton">
                                    <span>
                                        ...
                                        <span>Add Commercial Invoice </span>
                                    </span>
                                </a>
                                @endif
                                <a href="javascript:void(0)" class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" style="display: none" id="listButton">
                                    <span>
                                        <i class="fa fa-list"></i>
                                        <span>Manage Commercial Invoice</span>
                                    </span>
                                </a>
                            </div>
                        </div>
                        <commercial-invoice-entry-list
                                :permissions="{{ json_encode(permissionCheck()) }}"
                                :lc_lists="{{json_encode($lcLists)}}"
                                :cnf_agent_list="{{json_encode($cnfAgentList)}}"
                                :product_lists="{{json_encode($productLists)}}"
                                :country_lists="{{json_encode($countryLists)}}"
                                :measure_unit_lists="{{json_encode($measureUnitList)}}"
                        ></commercial-invoice-entry-list>
                        <!--begin::Form-->
                        <!--end::Form-->
                    </div>
...
@push('custom_scripts')
<script type="text/javascript" src="{!! asset('js/lc/commercial-invoice-entry.js') !!}"></script>
@endpush
