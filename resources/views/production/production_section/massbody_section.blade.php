@extends('master')
@section('title','Manage Massbody Section')
@section('content')
    <!-- BEGIN: Subheader -->
                    ...
                    <div class="m-portlet">
                        <div class="m-portlet__head">
                            ...
                            <div class="m-portlet__head-tools">
                                @if (in_array('massbody-section.store',permissionCheck()))
                                <a href="javascript:void(0)"  class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" id="addButton">
                                    <span>
                                        ...
                                        <span>Add Massbody Section</span>
                                    </span>
                                </a>
                                @endif
                                <a href="javascript:void(0)" class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" style="display: none" id="listButton">
                                    <span>
                                        <i class="fa fa-list"></i>
                                        <span>Manage Massbody Section</span>
                                    </span>
                                </a>
                            </div>
                        </div>
                        <massbody-section-list
                                :permissions="{{ json_encode(permissionCheck()) }}"
                                :rm_requisition_list="{{json_encode($rmRequisitionList)}}"
                                :recycle_chip="{{json_encode($recycleChip)}}"
                        ></massbody-section-list>
                        <!--begin::Form-->
                        <!--end::Form-->
                    </div>
...
@push('custom_scripts')
<script type="text/javascript" src="{!! asset('js/production/massbody-section.js') !!}"></script>
@endpush
