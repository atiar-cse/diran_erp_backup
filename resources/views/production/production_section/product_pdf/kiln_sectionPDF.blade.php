...
<body>
    ...
    <div style="text-align: center; display: block; margin: 10px 0;">
        <h4 style=" border-bottom: 1px solid #333; padding: 5px 0; width:120px; margin: 0 auto; ">Kiln Section</h4>
    </div>
    <br>

    <div style="display: block;">
        <div style="width: 50%;display:block; float:left;">
            <p> <span class="m-invoice__subtitle">Kiln No :</span>{{ $kiln['kiln_no'] }}</p>
        </div>
        <div style="width: 50%;display:block; float:left; text-align: right">
            <p><span class="m-invoice__subtitle">Kiln Date :</span>{{ $kiln['kiln_date'] }}</p>
        </div>
    </div>
    <br/>
    ...
    <table class="items" width="100%" style="font-size: 9pt; border-collapse: collapse; " cellpadding="8">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td align="center">{{ $row['kiln_weight'] }}</td>
                <td align="center">{{ $row['current_qty'] }}</td>
                <td align="center">{{ $row['receive_qty'] }}</td>
                <td align="center">{{ $row['trans_to_hv_qty'] }}</td>
                <td align="center">{{ $row['trans_to_hv_weight'] }}</td>
                <td align="center">{{ $row['reject_qty'] }}</td>
                <td align="center">{{ $row['reject_weight'] }}</td>
                <td align="center">{{ $row['remain_qty'] }}</td>
            </tr>
            @endforeach
            <tr>
                <td class="text-right m--font-danger" colspan="4" style="text-align: right">Total</td>
                <td class="text-right m--font-danger">{{ $kiln['total_receive_qty'] }}</td>
                <td class="text-right">{{ $kiln['total_trans_to_hv_qty'] }}</td>
                <td class="text-right">{{ $kiln['total_trans_weight'] }}</td>
                <td class="text-right">{{ $kiln['total_reject_weight'] }}</td>
                <td class="text-right">{{ $kiln['total_reject_qty'] }}</td>
                <td class="text-right">{{ $kiln['total_remain_qty'] }}</td>
            </tr>
