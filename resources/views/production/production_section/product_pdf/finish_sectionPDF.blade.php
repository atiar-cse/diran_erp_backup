...
<body>
    ...
    <div style="text-align: center; display: block; margin: 10px 0;">
        <h4 style=" border-bottom: 1px solid #333; padding: 5px 0; width:120px; margin: 0 auto; ">Finish Stock Section</h4>
    </div>
    <br>

    <div style="display: block;">
        <div style="width: 25.33%;display:block; float:left;">
            <p> <span class="m-invoice__subtitle">Finish No :</span>{{ $finish['finished_stock_no'] }}</p>
        </div>
        <div style="width: 20.33%;display:block; float:left; text-align: right;">
            <p><span class="m-invoice__subtitle">Date :</span>{{ $finish['date'] }}</p>
        </div>
    </div>
    <br/>
    ...
    <table class="items" width="100%" style="font-size: 9pt; border-collapse: collapse; " cellpadding="8">
        <thead>
            <tr>
                ...
                <th class="text-right">Rec.F Semi.F Qty</th>
                <th>Adj. Qty</th>
                <th class="text-right">Unit Price</th>
                <th class="text-right">	Trans to Store Qty</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td align="center">{{ $row['trans_to_store_qty'] }}</td>
            ...
            <tr>
                ...
                <td class="text-right m--font-danger">{{ $finish['total_receive_qty'] }}</td>
                <td class="text-right">{{ $finish['total_adj_qty'] }}</td>
                <td></td>
                <td class="text-right">{{ $finish['total_trans_to_store_qty'] }}</td>
                <td class="text-right">{{ $finish['total_amount'] }}</td>
                <td class="text-right">{{ $finish['total_reject_qty'] }}</td>
                <td class="text-right">{{ $finish['total_remain_qty'] }}</td>
            </tr>
