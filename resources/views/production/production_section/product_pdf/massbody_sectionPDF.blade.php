...
<body>
    ...
    <div style="text-align: center; display: block; margin: 10px 0;">
        <h4 style=" border-bottom: 1px solid #333; padding: 5px 0; width:120px; margin: 0 auto; ">Forming Section</h4>
    </div>
    <br>

    <div style="display: block;">
        <div style="width: 33.33%;display:block; float:left;">
            <p> <span class="m-invoice__subtitle">Forming No :</span>{{ $forming['forming_no'] }}</p>
        </div>
        <div style="width: 33.33%;display:block; float:left; text-align: center;">
            <p> <span class="m-invoice__subtitle">Forming Type :</span>{{ $forming['forming_type'] }}</p>

        </div>
        <div style="width: 33.33%;display:block; float:left; text-align: right;">
            <p><span class="m-invoice__subtitle">Forming Date :</span>{{ $forming['forming_date'] }}</p>
        </div>
    </div>
    <br/>
    ...
    <table class="items" width="100%" style="font-size: 9pt; border-collapse: collapse; " cellpadding="8">
        <thead>
            <tr>
                ...
                <th class="text-right">Trans to Shaping</th>
        ...
        <tbody>
            <!-- ITEMS HERE -->
            @foreach($forming['details'] as $row)
                @php
            ...
            <tr>
                ...
                <td align="center">{{ $row['roll_weight'] }}</td>
                <td align="center">{{ $row['current_qty'] }}</td>
                <td align="center">{{ $row['receive_qty'] }}</td>
                <td align="center">{{ $row['trans_to_shapping_qty'] }}</td>
                <td align="center">{{ $row['trans_to_shapping_weight'] }}</td>
                <td align="center">{{ $row['shapping_westage_qty'] }}</td>
                <td align="center">{{ $row['shapping_westage_weight'] }}</td>
                <td align="center">{{ $row['forming_remain_qty'] }}</td>
                {{--<td>{{ $row->product ? $row->product->product_name:'' }}</td>
                <td>{{ $row->remarks}}</td>
                <td>{{ $row->roll_weight}}</td>
                <td>{{ $row->current_qty}}</td>
                <td>{{ $row->receive_qty}}</td>
                <td>{{ $row->trans_to_shapping_qty}}</td>
                <td>{{ $row->trans_to_shapping_weight}}</td>
                <td>{{ $row->shapping_westage_qty}}</td>
                <td>{{ $row->shapping_westage_weight}}</td>
                <td>{{ $row->forming_remain_qty}}</td>--}}
            ...
            <tr>
                ...
                <td class="text-right m--font-danger">{{ $forming['total_trans_to_shapping_qty'] }}</td>
                <td class="text-right">{{ $forming['total_trans_to_shap_weight'] }}</td>
                <td class="text-right">{{ $forming['total_waste_qty'] }}</td>
                <td class="m--font-danger">{{ $forming['total_waste_weight'] }}</td>
                <td class="text-right">{{ $forming['total_remain_qty'] }}</td>
            </tr>
            <tr>
                <td class="text-right m--font-danger" colspan="7">Process Loss</td>
                <td class="text-right">{{ $forming['process_loss_percentage'] }}%</td>
                <td class="m--font-danger">{{ $forming['process_loss_weight'] }}</td>
                <td></td>
            </tr>

            <tr>
                <td class="text-right m--font-danger" colspan="8">Total Waste</td>
                <td class="m--font-danger">{{ $forming['weight_after_process_loss'] }}</td>
                <td></td>
            </tr>
