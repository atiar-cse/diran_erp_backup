...
<body>
    ...
    <div style="text-align: center; display: block; margin: 10px 0;">
        <h4 style=" border-bottom: 1px solid #333; padding: 5px 0; width:120px; margin: 0 auto; ">Glaze Section</h4>
    </div>
    <br>

    <div style="display: block;">
        <div style="width: 50%;display:block; float:left;">
            <p> <span class="m-invoice__subtitle">Glaze No :</span>{{ $glaze['glaze_no'] }}</p>
        </div>
        <div style="width: 50%;display:block; float:left; text-align: right">
            <p><span class="m-invoice__subtitle">Glaze Date :</span>{{ $glaze['glaze_date'] }}</p>
        </div>
    </div>
    <br/>
    ...
    <table class="items" width="100%" style="font-size: 9pt; border-collapse: collapse; " cellpadding="8">
        <thead>
            <tr>
                ...
                <th class="text-right">Trans Trans Weight </th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td align="center">{{ $row['glaze_weight'] }}</td>
                <td align="center">{{ $row['current_qty'] }}</td>
                <td align="center">{{ $row['receive_qty'] }}</td>
                <td align="center">{{ $row['trans_kiln_qty'] }}</td>
                <td align="center">{{ $row['trans_kiln_weight'] }}</td>
                <td align="center">{{ $row['waste_qty'] }}</td>
                <td align="center">{{ $row['waste_weight'] }}</td>
                <td align="center">{{ $row['remain_qty'] }}</td>
            </tr>
            @endforeach
            <tr>
                <td class="text-right m--font-danger" colspan="4" style="text-align: right">Total</td>
                <td class="text-right m--font-danger">{{ $glaze['total_receive_qty'] }}</td>
                <td class="text-right">{{ $glaze['total_trans_to_kiln_qty'] }}</td>
                <td class="text-right">{{ $glaze['total_trans_to_kiln_weight'] }}</td>
                <td class="m--font-danger">{{ $glaze['total_waste_qty'] }}</td>
                <td class="text-right">{{ $glaze['total_waste_weight'] }}</td>
                <td class="text-right">{{ $glaze['total_remain_qty'] }}</td>
            </tr>
            <tr>
                <td class="text-right m--font-danger" colspan="7" style="text-align: right">Process Loss</td>
                <td class="text-right">{{ $glaze['process_loss_percentage'] }}%</td>
                <td class="m--font-danger">{{ $glaze['process_loss_weight'] }}</td>
                <td></td>
            </tr>

            <tr>
                <td class="text-right m--font-danger" colspan="8" style="text-align: right">Total Waste</td>
                <td class="m--font-danger">{{ $glaze['weight_after_process_loss'] }}</td>
                <td></td>
            </tr>
