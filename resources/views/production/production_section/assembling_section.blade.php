@extends('master')
@section('title','Manage Assembling Section')
@section('content')
    <!-- BEGIN: Subheader -->
                    ...
                    <div class="m-portlet">
                        <div class="m-portlet__head">
                            ...
                            <div class="m-portlet__head-tools">
                                @if (in_array('assembling-section.store',$permissionCheck))
                                    <a href="javascript:void(0)"  class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" id="addButton">
                                        <span>
                                            ...
                                            <span>Add Assembling Section</span>
                                        </span>
                                    </a>
                                @endif
                                <a href="javascript:void(0)" class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" style="display: none" id="listButton">
                                    <span>
                                        <i class="fa fa-list"></i>
                                        <span>Manage Assembling Section</span>
                                    </span>
                                </a>
                            </div>
                        </div>
                        <assembling-section-list
                                :permissions="{{ json_encode(permissionCheck()) }}"
                                :finish_product_lists="{{json_encode($finishProductList)}}"
                                :product_lists="{{json_encode($productList)}}"
                                :warehouse_lists="{{json_encode($warehouse)}}"
                                :cost_center_lists="{{json_encode($costCenterLists)}}"
                        ></assembling-section-list>
                        <!--begin::Form-->
                        <!--end::Form-->
                    </div>
...
@push('custom_scripts')
<script type="text/javascript" src="{!! asset('js/production/assembling-section.js') !!}"></script>
@endpush
