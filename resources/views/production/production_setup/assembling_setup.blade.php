@extends('master')
@section('title','Manage Assembling Setup')
@section('content')
    <!-- BEGIN: Subheader -->
                    ...
                    <div class="m-portlet">
                        <div class="m-portlet__head">
                            ...
                            <div class="m-portlet__head-tools">
                                @if (in_array('assembling-setup.store',permissionCheck()))
                                <a href="javascript:void(0)"  class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" id="addButton">
                                    <span>
                                        ...
                                        <span>Add Assembling Setup</span>
                                    </span>
                                </a>
                                @endif
                                <a href="javascript:void(0)" class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air" style="display: none" id="listButton">
                                    <span>
                                        <i class="fa fa-list"></i>
                                        <span>Manage Assembling Setup</span>
                                    </span>
                                </a>
                            </div>
                        </div>
                        <assembling-setup-list
                                :permissions="{{ json_encode(permissionCheck()) }}"
                                :finishing_product_list="{{json_encode($finishingProductList)}}"
                                :fitting_product_list="{{json_encode($fittingProductList)}}"
                        ></assembling-setup-list>
                        <!--begin::Form-->
                        <!--end::Form-->
                    </div>
...
@push('custom_scripts')
<script type="text/javascript" src="{!! asset('js/production/assembling-setup.js') !!}"></script>
@endpush
