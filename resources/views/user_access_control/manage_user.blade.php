        ...
        <div class="m-portlet m-portlet--mobile">
            <div class="m-portlet__head">
                ...
                <div class="m-portlet__head-tools">
                    <a href="{{ URL::to('user/create') }}"  class="btn btn-sm btn-brand m-btn--icon m-btn--wide m-btn--sm m--margin-right-10 m-btn--air">
                        <span>
            ...
            <div class="m-portlet__body">
                ...
                <table class="table table-striped- table-bordered table-hover " id="m_table_1">
                    ...
                    @if(isset($userList) && count($userList))
                        ...
                        <tr class="{{$row->id}}">
                            <td>{{ ($key+1) }}</td>
                            <td><img src="uploads/user_photo/{!! $row->user_photo !!}" style="width:40px; height:40px; border-radius: 100px; margin-top: -6px"></td>
                            <td>{{ $row->user_name }}</td>
                            <td>{{ $row->email }}</td>
                            ...
                            <td>
                                <a href="{{ URL::to('user/' . $row->id . '/edit') }}" class="btn btn-outline-success m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air" data-offset="-20px -20px" data-toggle="m-tooltip" data-placement="top" title="Edit"><i class="la la-edit"></i></a>
                                <a href="{!! route('user.destroy',$row->id )!!}" data-token="{!! csrf_token() !!}" data-id="{!! $row->id !!}" class="btn btn-outline-danger m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air deleteBtn" data-offset="-20px -20px" data-toggle="m-tooltip" data-placement="top" title="Delete"><i class="la la-trash-o"></i></a>
                            </td>
                        </tr>
                        @endforeach
                    @endif
